...
import {
  CAT_COLS, DayPersonCollapse, isHotelRow as _isHotelRow, type JournalMode,
...
// 共用渲染元件（CAT_COLS / JournalMode / ShiftTag / DayPersonCollapse / isHotelRow）
// 已抽至 @/components/WorkJournal/shared，與 LuqunRepair「未指定工作日誌」TAB 共用（行為零變更）。
...
function WorkJournalTab({ onStatsChange }: { onStatsChange?: (s: WJStats) => void }) {
    ...
    try {
      ...
      if (mode === 'single') {
        ...
        const [journal, shifts] = await Promise.all([
          fetchWorkJournalDaily(year, month, day, 'named'),
        ...
        const [journal, shifts] = await Promise.all([
          fetchWorkJournalRange(from, to, 'named'),
        ...
        // 未指定：只顯示「未指定」人員工單；不抓班表（未指定無班表）、不更新上方摘要卡片（卡片維持具名口徑）
        if (unSubMode === 'single') {
          const journal = await fetchWorkJournalDaily(year, month, day, 'unassigned')
          setSingleData(journal)
          setRangeData(null)
          setShiftMapByDate({})
          setDateActiveKeys([])
          ...
          if (!from) return
          const journal = await fetchWorkJournalRange(from, to, 'unassigned')
          setRangeData(journal)
          setSingleData(null)
          setShiftMapByDate({})
          setDateActiveKeys(journal.days.map((_, i) => `day-${i}`))
  ...
  }, [mode, year, month, day, rangeDates, monthDate, personSubMode, personRangeDates, personMonthDate, personFilter, unSubMode])
  ...
  // 日期 pickers
  const renderPickers = () => {
    // 「未指定」模式：以子模式決定 picker 樣式，前面多一顆子模式切換
            ...
            return (
              <Button
                ...
                onClick={() => downloadFile(
                  getJournalExcelUrl(from, to, exportPerson, scope),
                  `工作日誌_${label}.xlsx`,
                )}
              >
                匯出 Excel
