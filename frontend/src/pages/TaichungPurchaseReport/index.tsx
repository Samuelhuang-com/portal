/**
 * 台中核准請購單月報表（2026-09-16 完整複製自 pages/PurchaseReport，資料來源改 /taichung-*-report）
 *
 * TAB 順序：
 *   TAB-1  請購單清單（訂單級）
 *   TAB-2  請購單月報明細（品項級）
...
import {
  ...
  type AuditAnomaly,
  type AuditSummary,
} from '@/api/taichungPurchaseReport'
import {
  getClaimOrders,
  getClaimOrderDetail,
...
const PAGE_SIZE = 50
const STORAGE_KEY = 'taichung_purchase_report_year_month'
...
export default function TaichungPurchaseReportPage() {
  ...
  const loadCombinedSummary = useCallback(() => {
    ...
    getCombinedSummary({ year_month: yearMonthFrom ? undefined : yearMonth, year_month_from: yearMonthFrom, year_month_to: yearMonthTo, company: '台中', department: deptFilter })
  ...
  const loadCombinedDeptStats = useCallback(() => {
    ...
    getCombinedDepartments({ year_month: yearMonthFrom ? undefined : yearMonth, year_month_from: yearMonthFrom, year_month_to: yearMonthTo, company: '台中' })
  ...
  const handleExportPurchase = async () => {
    ...
    try {
      ...
      a.download = `台中核准請購單月報表_${label}${deptFilter ? '_' + deptFilter : ''}.xlsx`
  ...
  const handleExportClaim = async () => {
    ...
    try {
      ...
      const url = URL.createObjectURL(new Blob([res.data]))
      const a = document.createElement('a'); a.href = url
      a.download = `台中核准請款單月報表_${label}${deptFilter ? '_' + deptFilter : ''}.xlsx`
      a.click(); URL.revokeObjectURL(url)
    } catch (err: any) {
      // 404 ＝ 篩選條件下沒有資料，這不是錯誤，用提示而非錯誤訊息
      if (err?.response?.status === 404) message.info('無資料可匯出')
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        <Title level={4} style={{ margin: 0 }}>
          {isPurchaseTab ? '台中核准請購單月報表' : isClaimTab ? '台中核准請款單月報表' : isCombinedTab ? '部門統計' : '資料異常稽核'}
        </Title>
          ...
          {selectedOrder && (
            <>
              <Descriptions bordered size="small" column={2} style={{ marginBottom: 16 }}>
                ...
                {/* ── 台中額外欄位（2026-09-16）── */}
                <Descriptions.Item label="主旨" span={2}>{selectedOrder.order.subject || '-'}</Descriptions.Item>
                <Descriptions.Item label="31事由" span={2}>{selectedOrder.order.reason_31 || '-'}</Descriptions.Item>
                <Descriptions.Item label="擬定廠商" span={2}>
                  {selectedOrder.order.final_vendor_name || '-'}
                  {selectedOrder.order.final_vendor_no && (
                    <Tag style={{ marginLeft: 8 }}>{selectedOrder.order.final_vendor_no}</Tag>
                  )}
                </Descriptions.Item>
                <Descriptions.Item label="擬定受款者">{selectedOrder.order.final_payee || '-'}</Descriptions.Item>
                <Descriptions.Item label="幣別">{selectedOrder.order.currency || '-'}</Descriptions.Item>
                <Descriptions.Item label="擬定受款銀行">{selectedOrder.order.final_bank_name || '-'}</Descriptions.Item>
                <Descriptions.Item label="擬定銀行代號">{selectedOrder.order.final_bank_code || '-'}</Descriptions.Item>
                <Descriptions.Item label="擬定匯款帳號" span={2}>{selectedOrder.order.final_bank_account || '-'}</Descriptions.Item>
                ...
                <>
                  ...
                  <Table
                    ...
                    columns={[
                      ...
                      { title: '應稅金額', dataIndex: 'taxable_amount', width: 90, align: 'right' as const, render: fmt },
                    ]}
                  />
                </>
              ) : (
          ...
          {selectedClaimOrder && (
            <>
              <Descriptions bordered size="small" column={2} style={{ marginBottom: 16 }}>
                ...
                {/* ── 台中額外欄位（2026-09-16）── */}
                <Descriptions.Item label="總金額">{fmt(selectedClaimOrder.order.total_amount)}</Descriptions.Item>
                <Descriptions.Item label="沖預支">{fmt(selectedClaimOrder.order.advance_offset)}</Descriptions.Item>
                <Descriptions.Item label="幣別">{selectedClaimOrder.order.currency || '-'}</Descriptions.Item>
                <Descriptions.Item label="付款銀行">{selectedClaimOrder.order.payment_bank || '-'}</Descriptions.Item>
