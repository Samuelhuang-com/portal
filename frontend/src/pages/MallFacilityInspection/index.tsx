/**
 * 春大直商場工務巡檢 — 模組統計 Dashboard
 *
 * 完全對齊 portal 現有「保全巡檢統計 Dashboard」(SecurityDashboard) 的頁面規格：
 *   - 全體 KPI 卡片（4 個）
 *   - 各 Sheet 今日統計表
 *   - Tabs：今日統計 / 異常清單 / 趨勢分析
 *
  ...
  const sheetCols = [
    {
      ...
      render: (v: string, row: SheetStats) => (
        <Button
          ...
          onClick={() => navigate(`/mall-facility-inspection/${row.key}`)}
        >
          {v}
        </Button>
      ),
  ...
  const IssuesTab = (
    <div>
      {abnormalAll > 0 ? (
        <Table
          dataSource={sheets
            .filter((s) => s.abnormal_items + s.pending_items > 0)
            .map((s) => ({
              key:     s.key,
              title:   s.title,
              abnormal: s.abnormal_items,
        ...
        <Alert message={`${targetDate} 無異常記錄`} type="success" showIcon />
      )}
    </div>
  )
      ...
      <Row align="middle" justify="space-between" style={{ marginBottom: 16 }}>
        <Col>
          <Title level={4} style={{ margin: 0, color: '#1B3A5C' }}>
            <DashboardOutlined /> {NAV_PAGE.mallFacilityDashboard}
          </Title>
        </Col>
        <Col>
          <Space>
