/**
 * 整棟工務每日巡檢 - RF  批次明細頁（唯讀）【寬表格 Pivot 架構】
 * Route: /mall/rf-inspection/:batchId
 */
...
import { fetchRFBatchDetail, fetchRFItemHistory } from '@/api/rfInspection'
import type {
  RFInspectionBatchDetail, RFInspectionItem, RFInspectionResultStatus,
  RFInspectionItemHistory, RFInspectionDailySummary,
...
function ItemHistoryDrawer({
  ...
  open: boolean
  onClose: () => void
  historyData: RFInspectionItemHistory | null
  loading: boolean
  ragicId?: string
}) {
  ...
  const selectedDayData: RFInspectionDailySummary | undefined = selectedDay
    ? daily_summary.find(ds => ds.inspection_date === selectedDay)
    : undefined

  function DayCell({ ds }: { ds: RFInspectionDailySummary }) {
        ...
        <Space>
          ...
          {ragicId && (
            <Tooltip title="在 Ragic 查看此批次原始紀錄">
              <a
                href={`https://ap12.ragic.com/soutlet001/full-building-inspection/1/${ragicId}`}
                target="_blank"
...
// Main Component
// ══════════════════════════════════════════════════════════════════════════════
export default function RFInspectionDetailPage() {
  const { batchId } = useParams<{ batchId: string }>()
  const navigate    = useNavigate()

  const [detail, setDetail]   = useState<RFInspectionBatchDetail | null>(null)
  ...
  const [historyData, setHistoryData]       = useState<RFInspectionItemHistory | null>(null)

  const loadDetail = useCallback(async () => {
    if (!batchId) return
    setLoading(true)
    try {
      const data = await fetchRFBatchDetail(batchId)
      setDetail(data)
    } catch {
  ...
  async function openHistory(itemName: string) {
    ...
    setHistoryData(null)
    try {
      const data = await fetchRFItemHistory(itemName, 30)
      setHistoryData(data)
    } catch {
  ...
  // ── 表格欄位 ──────────────────────────────────────────────────────────────
  const columns: ColumnsType<RFInspectionItem> = [
    {
      title: '項次',
      dataIndex: 'seq_no',
      width: 60,
    ...
    },
    {
      ...
      width: 100,
      align: 'center',
      render: (v: RFInspectionResultStatus, rec) => {
