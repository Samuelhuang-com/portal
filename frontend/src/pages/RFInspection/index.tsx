...
import { fetchRFStats, fetchRFBatches, syncRFFromRagic } from '@/api/rfInspection'
import type {
  RFInspectionStats, RFInspectionBatchListItem, RFInspectionItem,
...
function deriveBatchStatus(kpi: RFInspectionBatchListItem['kpi']): { label: string; color: string } {
...
// ── 主元件 ────────────────────────────────────────────────────────────────────
export default function RFInspectionPage() {
  const navigate = useNavigate()
  const [activeTab, setActiveTab] = useState('dashboard')
  const [stats, setStats]     = useState<RFInspectionStats | null>(null)
  const [batches, setBatches] = useState<RFInspectionBatchListItem[]>([])
  ...
  const loadBatches = useCallback(async () => {
    setLoading(true)
    try {
      const data = await fetchRFBatches({ year_month: yearMonth })
      setBatches(data)
    } catch {
                  ...
                  },
                  {
                    ...
                    render: (_, row) => (
                      <Button type="link" size="small" icon={<RightOutlined />}
                        onClick={() => navigate(`/mall/rf-inspection/${row.batch_ragic_id}`)} />
                    ),
                  },
                ]}
              />
          ...
          <Row align="middle" justify="space-between">
            ...
            <Col>
              <Button
                ...
                onClick={() => navigate(`/mall/rf-inspection/${stats.latest_batch!.ragic_id}`)}
  ...
  // ── 巡檢紀錄 Tab ──────────────────────────────────────────────────────────
  const batchColumns: ColumnsType<RFInspectionBatchListItem> = [
    {
      title: '巡檢日期',
    ...
    },
    {
      ...
      render: (v, row) => (
        <Button type="link" style={{ padding: 0, fontWeight: 600 }}
          onClick={() => navigate(`/mall/rf-inspection/${row.batch.ragic_id}`)}>
  ...
  const ListTab = (
    <div>
      ...
      <Table<RFInspectionBatchListItem>
        dataSource={batches}
      ...
      <Row align="middle" justify="space-between" style={{ marginBottom: 16 }}>
        <Col>
          <Title level={4} style={{ margin: 0, color: '#1B3A5C' }}>
            <SafetyOutlined /> {NAV_PAGE.rfInspection}
          </Title>
        </Col>
        <Col>
          <Button
