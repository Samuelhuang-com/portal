...
import {
  fetchMallFIDailyForm,
  type MallFIDailyFormRow,
...
type RowWithKey = MallFIDailyFormRow & { _key: string }

// ── 主元件 ────────────────────────────────────────────────────────────────────

export default function MallDailyInspectionFormTab() {
  const now = dayjs()
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await fetchMallFIDailyForm(year, month, inspDate)
  ...
  const rowClassName = (row: RowWithKey): string => {
    if (row.abnormal) return 'mall-daily-form-row--abnormal'
    if (!row.matched) return 'mall-daily-form-row--unchecked'
    return ''
  }
  ...
  return (
    <div>
      {/* CSS */}
      <style>{`
        .mall-daily-form-row--abnormal td  { background-color: #fff1f0 !important; }
        .mall-daily-form-row--unchecked td { background-color: #fffbe6 !important; }
        .mall-daily-form-table .ant-table-thead > tr > th {
        ...
        .mall-daily-form-table .ant-table-cell {
      ...
      {/* ── 表格 ── */}
      <Table<RowWithKey>
        className="mall-daily-form-table"
        dataSource={rows}
        rowKey="_key"
        columns={columns}
        loading={loading}
