...
import {
  fetchMallPMStats, fetchMallPMBatches, fetchMallPMBatchDetail,
...
// ── 保養項目明細（KPI 卡片 Drawer 用，2026-07-13 新增，比照 full_bldg_pm 同名函式）──────
  ...
  return (
    <>
      ...
      <Row gutter={[12, 12]} style={{ marginBottom: 16 }}>
        <Col span={24}>
          <Typography.Text type="secondary" style={{ fontSize: 12 }}>▸ {currLabel}統計</Typography.Text>
        </Col>
        {[
      ...
      {data.period_total > 0 && (
        <Card size="small" style={{ marginBottom: 16 }}>
          <Row align="middle" gutter={16}>
            <Col flex="80px"><Typography.Text strong>{currLabel}完成率</Typography.Text></Col>
            <Col flex="auto">
              <Progress
    ...
    { title: '完成數', dataIndex: 'completed', width: 90, align: 'right' },
    {
      ...
      render: (_, r) => (
        r.total === 0
          ...
          : <Progress percent={r.rate ?? 0} size="small" strokeColor={{ from: '#4BA8E8', to: '#52C41A' }} format={() => fmtRate(r.rate)} />
      ),
    },
  ]
  return (
    <Card title={title} size="small" style={{ marginBottom: 16 }}>
      <Table<PMSubPeriodBreakdown> dataSource={rows} rowKey="label" columns={cols} pagination={false} size="small" />
    </Card>
  )
}

interface IncompleteTableProps { items: PMIncompleteItem[] }
function IncompleteTable({ items }: IncompleteTableProps) {
  ...
  return (
    <Card title={<><WarningOutlined style={{ color: '#C0392B' }} /> 未完成事項說明</>} size="small">
      {items.length === 0 ? (
...
interface QuarterSummary {
  q: number; months: string; total: number; completed: number; rate: number | null
}
function deriveQuarterSummaries(matrix: PMYearMatrix): QuarterSummary[] {
  return [1, 2, 3, 4].map((q) => {
    ...
    return { q, months: slice.map((m) => `${m.month}月`).join(' '), total, completed,
             rate: total > 0 ? Math.round((completed / total) * 1000) / 10 : null }
  })
}

function QuarterSelectorCards({ matrix, selectedQ, onSelect }: {
  matrix: PMYearMatrix; selectedQ: number; onSelect: (q: number) => void
}) {
        ...
        return (
          <Col span={6} key={q}>
            <Card
              ...
              onClick={() => onSelect(q)}
              style={{ cursor: 'pointer', border: selected ? '2px solid #4BA8E8' : '1px solid #d9d9d9',
                       background: selected ? '#f0f8ff' : undefined, textAlign: 'center' }}
            >
              <Typography.Title level={4} style={{ margin: '0 0 2px', color: '#1B3A5C' }}>Q{q}</Typography.Title>
              <Typography.Text type="secondary" style={{ fontSize: 12 }}>{months}</Typography.Text>
              <Divider style={{ margin: '8px 0' }} />
              <Row gutter={4} justify="center">
                <Col span={8}><div style={{ fontSize: 11, color: '#999' }}>項目</div><div style={{ fontSize: 18, fontWeight: 600, color: '#1B3A5C' }}>{total || '—'}</div></Col>
                <Col span={8}><div style={{ fontSize: 11, color: '#999' }}>完成</div><div style={{ fontSize: 18, fontWeight: 600, color: '#52C41A' }}>{completed || '—'}</div></Col>
                <Col span={8}><div style={{ fontSize: 11, color: '#999' }}>完成率</div><div style={{ fontSize: 16, fontWeight: 600, color: rateColor }}>{fmtRate(rate)}</div></Col>
              </Row>
            </Card>
          </Col>
        )
...
function YearMatrixTable({
  ...
  const summaryValues: Record<string, unknown> = {
    ...
    carry_over_rate:         totalPrevCarryOver > 0
                               ? Math.round(totalPrevResolved / totalPrevCarryOver * 1000) / 10 : null,
  ...
  const futureCell = () => <Typography.Text type="secondary" style={{ fontSize: 18 }}>—</Typography.Text>
  ...
  const renderCell = (v: unknown, row: Record<string, unknown>, isTotal = false, monthNum = 0, monthLabel = '') => {
    ...
    if (metric.isText) {
      ...
      return (
        <Tooltip title={<span style={{ whiteSpace: 'pre-wrap' }}>{text}</span>} placement="topLeft">
          <Typography.Text ellipsis style={{ fontSize: 17, display: 'block', maxWidth: 90, cursor: 'pointer' }}>
            {text}
          </Typography.Text>
        </Tooltip>
      )
    ...
    if (metric.isRate) {
      ...
      return (
        <Typography.Text style={{ fontSize: 18, color: rate === null ? '#999' : rate >= 80 ? '#52C41A' : rate >= 50 ? '#FAAD14' : '#FF4D4F' }}>
          {fmtRate(rate)}
        </Typography.Text>
      )
    }
  ...
  const cols: ColumnsType<Record<string, unknown>> = [
    ...
    ...data.months.map((m) => ({
      ...
      dataIndex: `m${m.month}`, width: 90, align: 'center' as const,
      onCell: (row: Record<string, unknown>) => ({
        style: { background: row['_isSep'] ? '#fafafa' : isFuture(m.month) ? '#fafafa' : undefined, verticalAlign: 'top' },
      }),
    ...
    })),
    {
      ...
      dataIndex: '_total', width: 100, align: 'center' as const, fixed: 'right' as const,
      onCell: (row: Record<string, unknown>) => ({
        style: { background: row['_isSep'] ? '#fafafa' : '#f6f8fc', verticalAlign: 'top', borderLeft: '2px solid #d9e4f0' },
      }),
...
export default function MallPeriodicMaintenancePage() {
  ...
  // ── Dashboard KPI 卡片明細 Drawer state（2026-07-13 新增，比照 full_bldg_pm）───────
  ...
  // ── KPI 明細內「保養項目」次層 Drawer state（共用 MallPMItemWorklogDrawer）─────────
  ...
  // ── Dashboard KPI 卡片明細 Drawer（2026-07-13 新增，比照 full_bldg_pm openKpiDrawer）──
  ...
  const loadBatches = useCallback(async () => {
    setLoading(true)
    try { const data = await fetchMallPMBatches(year); setBatches(data) }
    catch { message.error('載入批次清單失敗') }
    finally { setLoading(false) }
  }, [year])

  const loadMonthlyStats = useCallback(async () => {
    setMonthlyLoading(true)
    try { const data = await fetchMallPMPeriodStats({ period_type: 'month', year: monthlyYear, month: monthlyMonth, frequency_type: 'monthly' }); setMonthlyData(data) }
    catch { message.error('載入月統計失敗') }
    finally { setMonthlyLoading(false) }
  }, [monthlyYear, monthlyMonth])

  const loadYearMatrix = useCallback(async () => {
    setMatrixLoading(true)
    try { const data = await fetchMallPMYearMatrix(monthlyYear, 'monthly'); setMatrixData(data) }
    catch { message.error('載入年度矩陣失敗') }
    finally { setMatrixLoading(false) }
  }, [monthlyYear])

  const loadQuarterlyMatrix = useCallback(async () => {
    setQuarterlyMatrixLoading(true)
    try { const data = await fetchMallPMYearMatrix(quarterlyYear, 'quarterly'); setQuarterlyMatrixData(data) }
    catch { message.error('載入季度概覽失敗') }
    finally { setQuarterlyMatrixLoading(false) }
  }, [quarterlyYear])

  const loadQuarterlyStats = useCallback(async () => {
    setQuarterlyLoading(true)
    try { const data = await fetchMallPMPeriodStats({ period_type: 'quarter', year: quarterlyYear, quarter: quarterlyQuarter, frequency_type: 'quarterly' }); setQuarterlyData(data) }
    catch { message.error('載入季統計失敗') }
  ...
  const loadYearlyMatrix = useCallback(async () => {
    ...
    try { const data = await fetchMallPMYearMatrix(yearlyYear, 'yearly'); setYearlyMatrixData(data) }
    catch { message.error('載入年度矩陣失敗') }
    finally { setYearlyMatrixLoading(false) }
  }, [yearlyYear])

  const loadYearlyStats = useCallback(async () => {
    setYearlyLoading(true)
    try { const data = await fetchMallPMPeriodStats({ period_type: 'year', year: yearlyYear, frequency_type: 'yearly' }); setYearlyData(data) }
    catch { message.error('載入年統計失敗') }
  ...
  const [annualDrawerCell,  setAnnualDrawerCell]  = useState<{
    ...
    cell?: MallPMScheduleAnnualMatrix['rows'][0]['cells'][0]
  } | null>(null)
  ...
  const openAnnualDrawer = useCallback(async (
    ...
    setAnnualDrawerCell({ scheduleId, status, month, row, cell })
    setAnnualDrawerOpen(true)
    setAnnualCellDetail(null)
    if (scheduleId) {
      setAnnualCellLoading(true)
      try {
        // ⚠️ 2026-08-27：顯示月份 ≠ 批次月份（年度計劃表改以「執行月份」歸屬）。
        // mall_pm_schedule.year_month 存的是原批次月份，這裡必須用 origin_month 查，
        // 否則排程明細永遠找不到。
        const originMonth = cell?.entries?.[0]?.origin_month ?? month
        const res = await getMallScheduleList({
          year_month: `${annualYear}/${String(originMonth).padStart(2, '0')}`,
        })
  ...
  const catChartData = (stats?.category_stats ?? []).map(c => ({
    name: c.category, 已完成: c.completed, 未完成: c.total - c.completed, 完成率: c.rate,
  }))
  ...
  const DashboardTab = (
    <div>
      <Row gutter={8} align="middle" style={{ marginBottom: 16 }}>
        <Col><Text type="secondary" style={{ marginRight: 4 }}>查詢月份：</Text></Col>
        <Col>
          <Select value={dashYear} onChange={(v) => setDashYear(v)} options={yearOptions} style={{ width: 100 }} />
        </Col>
        <Col>
          <Select value={dashMonth} onChange={(v) => setDashMonth(v)} options={monthOptions} style={{ width: 85 }} />
        </Col>
        <Col>
          <Button icon={<ReloadOutlined />} onClick={loadDashboard} loading={loading}>重新整理</Button>
        </Col>
      </Row>

      <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
        {[
          { title: `${dashYear}/${String(dashMonth).padStart(2, '0')} 有效項目`, value: kpi?.current_month_total ?? 0, suffix: '筆', icon: <ToolOutlined />, color: '#1B3A5C', metric: 'current_month_total' as const },
          { title: `已完成（${kpi?.completion_rate ?? 0}%）`, value: kpi?.completed ?? 0, suffix: '筆', icon: <CheckCircleOutlined />, color: '#52C41A', metric: 'completed' as const },
          { title: '逾期件數', value: kpi?.overdue ?? 0, suffix: '筆', icon: <WarningOutlined />, color: '#C0392B', metric: 'overdue' as const },
          { title: '異常待追蹤', value: kpi?.abnormal ?? 0, suffix: '筆', icon: <ExclamationCircleOutlined />, color: '#722ED1', metric: 'abnormal' as const },
        ].map((card) => (
          <Col flex={1} style={{ minWidth: 140 }} key={card.title}>
            <Card size="small" hoverable style={{ height: '100%' }} onClick={() => openKpiDrawer(card.metric, card.title)}>
        ...
        <Col flex={1} style={{ minWidth: 140 }}>
          <Card size="small" hoverable style={{ height: '100%' }} onClick={() => openKpiDrawer('planned_minutes', '預估工時明細')}>
            <Statistic title="預估工時" value={Math.round((kpi?.planned_minutes ?? 0) / 60 * 10) / 10} suffix="小時"
              prefix={<span style={{ color: '#4BA8E8' }}><ClockCircleOutlined /></span>}
              valueStyle={{ color: '#4BA8E8', fontSize: 28 }} />
          </Card>
        </Col>
        <Col flex={1} style={{ minWidth: 140 }}>
          <Card size="small" hoverable style={{ height: '100%' }} onClick={() => openKpiDrawer('actual_minutes', '保養時間明細')}>
            <Statistic title="保養時間" value={Math.round((kpi?.actual_minutes ?? 0) / 60 * 10) / 10} suffix="小時"
      ...
      {kpi && kpi.current_month_total > 0 && (
        <Card size="small" style={{ marginBottom: 16 }}>
          <Row align="middle" gutter={16}>
            ...
            <Col flex="auto">
              <Progress percent={kpi.completion_rate} strokeColor={{ from: '#4BA8E8', to: '#52C41A' }}
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
        <Col xs={24} lg={14}>
          <Card title={<><BarChartOutlined /> 各類別完成率</>} size="small" style={{ height: 300 }}>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie data={pieData} dataKey="count" nameKey="label" cx="50%" cy="50%"
                    innerRadius={55} outerRadius={85} label={({ label, count }) => `${label}:${count}`} labelLine={false}>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={12}>
          <Card title={<><WarningOutlined style={{ color: '#C0392B' }} /> 逾期項目（Top 10）</>} size="small">
            {(stats?.overdue_items ?? []).length === 0 ? (
              <Alert message="該月無逾期項目" type="success" showIcon />
            ) : (
              <Table<PMItem> dataSource={stats?.overdue_items ?? []} rowKey="ragic_id" size="small" pagination={false}
                columns={[
                  { title: '項目', dataIndex: 'task_name', ellipsis: true },
                  { title: '類別', dataIndex: 'category', width: 70 },
                  { title: '排定日', dataIndex: 'scheduled_date', width: 75, render: (v) => <Text type="danger">{v || '—'}</Text> },
                  { title: '', width: 50, render: (_, row) => (
                    <Button type="link" size="small" icon={<RightOutlined />}
                      onClick={() => navigate(`/mall/periodic-maintenance/${row.batch_ragic_id}`)} />
                  )},
                ]}
              />
            )}
        ...
        <Col xs={24} lg={12}>
          <Card title={<><ClockCircleOutlined style={{ color: '#FAAD14' }} /> 待執行項目（排定中）</>} size="small">
            {(stats?.upcoming_items ?? []).length === 0 ? (
              <Alert message="該月無待執行項目" type="info" showIcon />
            ) : (
              <Table<PMItem> dataSource={stats?.upcoming_items ?? []} rowKey="ragic_id" size="small" pagination={false}
                columns={[
                  { title: '項目', dataIndex: 'task_name', ellipsis: true },
          ...
          <Row align="middle" justify="space-between">
            ...
            <Col>
              <Button type="primary" icon={<RightOutlined />}
                onClick={() => navigate(`/mall/periodic-maintenance/${stats.current_batch!.ragic_id}`)}
  ...
  const MonthlyStatsTab = (
    <div>
      <Row gutter={8} align="middle" style={{ marginBottom: 12 }}>
        <Col><Typography.Text type="secondary" style={{ marginRight: 4 }}>年度：</Typography.Text></Col>
        <Col>
          <Select value={monthlyYear} onChange={(v) => { setMonthlyYear(v) }} options={yearNumOptions} style={{ width: 100 }} />
        </Col>
        <Col>
          <Button icon={<ReloadOutlined />} onClick={() => { loadYearMatrix(); loadMonthlyStats() }}
            loading={matrixLoading || monthlyLoading}>重新整理</Button>
        </Col>
        <Col>
          <Button icon={<ToolOutlined />} type="primary"
            style={{ background: 'linear-gradient(135deg, #667eea, #764ba2)', border: 'none' }}
            onClick={() => openCatalogModal('monthly')}>保養項目</Button>
        </Col>
      </Row>
      ...
      <Divider orientation="left" style={{ fontSize: 13, color: '#666' }}>單月鑽取</Divider>
      <Row gutter={8} align="middle" style={{ marginBottom: 12 }}>
        <Col><Typography.Text type="secondary" style={{ marginRight: 4 }}>查詢月份：</Typography.Text></Col>
        <Col>
          <Select value={monthlyMonth} onChange={(v) => setMonthlyMonth(v)} options={monthOptions} style={{ width: 85 }} />
        </Col>
        <Col>
          <Button icon={<ReloadOutlined />} onClick={loadMonthlyStats} loading={monthlyLoading}>查詢</Button>
        </Col>
      </Row>
  ...
  const QuarterlyStatsTab = (
    <div>
      <Row gutter={8} align="middle" style={{ marginBottom: 12 }}>
        <Col><Typography.Text type="secondary" style={{ marginRight: 4 }}>年度：</Typography.Text></Col>
        <Col>
          <Select value={quarterlyYear} onChange={(v) => setQuarterlyYear(v)} options={yearNumOptions} style={{ width: 100 }} />
        </Col>
        <Col>
          <Button icon={<ReloadOutlined />} onClick={() => { loadQuarterlyMatrix(); loadQuarterlyStats() }}
            loading={quarterlyMatrixLoading || quarterlyLoading}>重新整理</Button>
        </Col>
        <Col>
          <Button icon={<ToolOutlined />} type="primary"
            style={{ background: 'linear-gradient(135deg, #667eea, #764ba2)', border: 'none' }}
            onClick={() => openCatalogModal('quarterly')}>保養項目</Button>
        </Col>
      </Row>
      ...
      {quarterlyMatrixData && (
        <QuarterSelectorCards matrix={quarterlyMatrixData} selectedQ={quarterlyQuarter}
  ...
  const YearlyStatsTab = (
    <div>
      <Row gutter={8} align="middle" style={{ marginBottom: 16 }}>
        <Col><Typography.Text type="secondary" style={{ marginRight: 4 }}>查詢年度：</Typography.Text></Col>
        <Col>
          <Select value={yearlyYear} onChange={(v) => setYearlyYear(v)} options={yearNumOptions} style={{ width: 100 }} />
        </Col>
        <Col>
          <Button icon={<ReloadOutlined />} onClick={() => { loadYearlyMatrix(); loadYearlyStats() }}
            loading={yearlyMatrixLoading || yearlyLoading}>重新整理</Button>
        </Col>
        <Col>
          <Button icon={<ToolOutlined />} type="primary"
            style={{ background: 'linear-gradient(135deg, #667eea, #764ba2)', border: 'none' }}
            onClick={() => openCatalogModal('yearly')}>保養項目</Button>
        </Col>
      </Row>
  ...
  const batchColumns: ColumnsType<PMBatchListItem> = [
    {
      ...
      render: (v, row) => (
        <Button type="link" style={{ padding: 0, fontWeight: 600 }}
          onClick={() => navigate(`/mall/periodic-maintenance/${row.batch.ragic_id}`)}>{v}</Button>
      ),
    },
      ...
      sorter: (a, b) => a.batch.period_month.localeCompare(b.batch.period_month) },
    {
      ...
      render: (_, row) => { const s = deriveBatchStatus(row.kpi); const cfg = BATCH_STATUS_CFG[s]; return <Tag color={cfg.color}>{cfg.label}</Tag> },
    },
    {
      title: '完成率', width: 180,
      render: (_, row) => {
        ...
        return (
          <div>
            <Progress percent={completion_rate} size="small" strokeColor={{ from: '#4BA8E8', to: '#52C41A' }}
              format={() => `${completion_rate}%`} />
            <Text type="secondary" style={{ fontSize: 11 }}>{completed} / {total} 已完成/全部</Text>
          </div>
        )
      },
    },
  ...
  const matrixCellCfg = (status: string, year: number, month: number) => {
    const base = MATRIX_CELL_CFG[status as string] ?? MATRIX_CELL_CFG['no_frequency']
                ...
                {filteredAnnualRows.map(row=>(
                  <tr key={row.item_ragic_id} style={{borderBottom:'1px solid #eee'}}>
                    <td style={{...TD,whiteSpace:'nowrap'}}>
                      <Tag style={{fontSize:12}}>{row.category||'—'}</Tag>
                      {row.category_variants?.length>1 && (
                        <Tooltip title={`本項目跨月出現過 ${row.category_variants.length} 種類別：${row.category_variants.join('、')}（顯示最近月份的值）`}>
                          <span style={{color:'#FA8C16',fontSize:12,marginLeft:2,cursor:'help'}}>⚠</span>
                        </Tooltip>
                      )}
                    </td>
                    <td style={{...TD,whiteSpace:'nowrap'}}>
                    ...
                    <td style={{...TD,textAlign:'center',whiteSpace:'nowrap'}}>
                      ...
                      {row.frequency_variants?.length>1 && (
                    ...
                    {row.cells.map(cell=>{
                      ...
                      return (
                        <td key={cell.month} style={{...TD,textAlign:'center',background:cfg.bg,color:cfg.text,
                          ...
                          onClick={()=>clickable&&openAnnualDrawer(cell.schedule_id,cell.status,cell.month,row,cell)}>
                          <Tooltip title={
                            (MATRIX_TOOLTIP_LABEL[cell.status] ?? cell.status)
                            + (cell.count>1
                            ...
                            <div style={{lineHeight:1.2,position:'relative'}}>
                              {cfg.label}
                              {cell.count>1 && (
                                <span style={{position:'absolute',top:-2,right:2,fontSize:10,fontWeight:700,
                                  color:'#fff',background:cfg.text,borderRadius:8,padding:'0 4px',lineHeight:'14px'}}>
                                  ×{cell.count}
                                </span>
                              )}
                              {offSched && (
                                <div style={{fontSize:10,fontWeight:500,color:'#FA8C16',marginTop:1}}>
                                  非排定月
                                </div>
                              )}
                              {cell.count>1 ? (
                                <div style={{fontSize:11,fontWeight:500,color:cfg.text,opacity:0.85,marginTop:1}}>
          ...
          <Table
            ...
            columns={[
              ...
              { title: '狀態', dataIndex: 'status', width: 90, align: 'center' as const,
                render: (v: string) => (
                  <Tag color={SCHED_STATUS_CFG[v]?.color ?? 'default'}>
  ...
  const ListTab = (
    <div>
      <Row gutter={8} style={{ marginBottom: 16 }}>
        ...
        <Col>
          <Button icon={<ReloadOutlined />} onClick={loadBatches} loading={loading}>重新整理</Button>
        </Col>
      </Row>
      <Table<PMBatchListItem>
        dataSource={batches} rowKey={(r) => r.batch.ragic_id} columns={batchColumns}
    ...
    <div style={{ padding: '0 4px' }}>
      <Breadcrumb
        ...
        items={[
          ...
          { title: NAV_PAGE.mallPeriodicMaintenance },
        ]}
      />
      ...
      {/* Dashboard KPI 卡片明細 Drawer（2026-07-13 新增，比照 full_bldg_pm）───────── */}
      <Drawer
        open={kpiDrawerOpen}
        width={760}
...
function MatrixDetailModal({
  ...
  const [loading, setLoading] = useState(false)
  const [items,   setItems]   = useState<MallPMMatrixItem[]>([])
  const [total,   setTotal]   = useState(0)

  useEffect(() => {
    if (!open || !year) return
    setLoading(true)
    fetchMallPMMatrixItems({ year, month, metric, frequency_type: frequencyType || undefined })
  ...
  const columns: ColumnsType<MallPMMatrixItem> = [
