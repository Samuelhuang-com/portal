...
// ── 年度計劃矩陣儲存格樣式 ────────────────────────────────────────────────────
// ── 年度計劃表狀態樣式（2026-08-27 統一：三個保養模組與 PPT 同一套）────────────
// 待執行用同心圓 ◎（U+25CE，文字符號，可自由上紅／綠）。⭕ 是彩色 emoji、顏色寫死
// 紅色、CSS color 蓋不掉，而且沒有綠色版本，所以不能用。逾期維持 🔴 實心圓 ——
// 實心 vs 同心的形狀差異正好對應「已經逾期」與「還沒到期」。
const ANNUAL_CELL_STYLE: Record<string, { icon: string; bg: string; color: string }> = {
  ...
  scheduled:    { icon: '◎',  bg: '#fff1f0', color: '#C0392B' },   // 當月待執行
  future_due:   { icon: '◎',  bg: '#f0fbe3', color: '#389E0D' },   // 未來待執行
  // unscheduled 與 no_data 共用黃問號（2026-08-27 會議裁示：兩個狀態保留，符號合併）
  unscheduled:  { icon: '?',  bg: '#fffbe6', color: '#FAAD14' },
  no_data:      { icon: '?',  bg: '#fffbe6', color: '#FAAD14' },
  no_frequency: { icon: '∅',  bg: '#f5f5f5', color: '#ccc' },
  non_month:    { icon: '─',  bg: '#fafafa', color: '#aaa' },
}

// 待執行的紅／綠由「格子月份是不是當月」決定：當月＝紅◎（該做了），非當月＝綠◎
// （排定日期還沒到）。排定日期已過的格子後端會給 overdue，不會走到這裡。
const annualCellStyle = (status: string, year: number, month: number) => {
  const base = ANNUAL_CELL_STYLE[status] ?? ANNUAL_CELL_STYLE['no_frequency']
  if (status !== 'scheduled') return base
  const now = new Date()
  const isCurrent = year === now.getFullYear() && month === now.getMonth() + 1
  return isCurrent ? base : ANNUAL_CELL_STYLE['future_due']
}

// 圖例順序固定（2026-08-27 會議裁示）：「非本月」排最後；「未排定」與「應做未排」
// 合併成一項黃問號，避免兩個名稱看起來像兩件事。
const ANNUAL_LEGEND: { key: string; label: string }[] = [
...
export default function FullBuildingMaintenancePage() {
  ...
  const openAnnualCell = useCallback(async (
    ...
    if (status === 'non_month' || status === 'no_frequency' || status === 'future_due') return
    ...
    if (scheduleId) {
      ...
      try {
        ...
        const ym = `${annualYear}/${String(originMonth).padStart(2, '0')}`
  ...
  useEffect(() => {
    ...
    if (activeTab === 'annual') loadAnnualMatrix()
  }, [activeTab, loadBatches, loadMonthlyStats, loadQuarterlyStats, loadYearlyStats, loadYearMatrix, loadQuarterlyMatrix, loadYearlyMatrix, loadSchedule, loadOverdue, loadAnnualMatrix, loadAnnualMatrix])
  ...
  const ANNUAL_STATUS_LABEL: Record<string, string> = {
    ...
    scheduled: '當月待執行', future_due: '未來待執行',
    unscheduled: '應做未排', no_data: '應做未排',
    no_frequency: '頻率未設', non_month: '非本月',
  }
  const ANNUAL_TOOLTIP_LABEL: Record<string, string> = {
    completed: '已完成', overdue: '逾期', in_progress: '進行中',
    scheduled: '待執行（已排定日期，尚未到期）',
    future_due: '未來待執行（依「執行月份」到期，尚未排定日期，也還沒有該月批次）',
  ...
  const AnnualTab = (
    <div>
      ...
      <Row gutter={4} style={{ marginBottom: 12 }}>
        {ANNUAL_LEGEND.map(({ key, label }) => {
          const v = ANNUAL_CELL_STYLE[key]
          return (
            <Col key={key}>
              ...
              <tbody>
                {filteredAnnualRows.map(row => (
                  <tr key={row.item_ragic_id} style={{ borderBottom:'1px solid #eee' }}>
                    ...
                    {row.cells.map(cell => {
                      const cs = annualCellStyle(cell.status, annualYear, cell.month)
  ...
  return (
    <>
      ...
      >
        ...
        {(annualDrawerCell?.cell?.entries ?? []).some(e => e.off_schedule) && (
          <Alert type="warning" showIcon style={{ marginBottom: 12 }}
        ...
        {!annualCellLoading && (annualDrawerCell?.cell?.count ?? 0) > 1 && (
          <>
            <Alert type="info" showIcon style={{ marginBottom: 12 }}
              message={`本格共 ${annualDrawerCell!.cell!.count} 筆記錄，格內顯示的是彙總狀態`} />
            <Table
              size="small"
