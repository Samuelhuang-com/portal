/**
 * 全棟例行維護 — 批次明細頁（唯讀，資料來源全部為 Ragic）
 * Route: /mall/full-building-maintenance/:batchId
 */
...
// Main Component
// ══════════════════════════════════════════════════════════════════════════════
export default function FullBuildingMaintenanceDetailPage() {
  ...
  const loadDetail = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const data = await fetchFullBldgPMBatchDetail(batchId)
      setDetail(data)
    } catch {
  ...
  async function openHistory(taskName: string) {
    ...
    setHistoryData(null)
    try {
      const data = await fetchFullBldgPMTaskHistory(taskName)
      setHistoryData(data)
    } catch {
  ...
  return (
    <div style={{ padding: '0 0 32px' }}>
      <Breadcrumb
        ...
        items={[
          { title: '商場管理' },
          { title: <a onClick={() => navigate('/mall/full-building-maintenance')}>全棟例行維護</a> },
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <Space>
          <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/mall/full-building-maintenance')}>
            返回列表
          </Button>
