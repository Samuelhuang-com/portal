/**
 * 整棟工務每日巡檢 - B2F  批次明細頁（唯讀）【寬表格 Pivot 架構】
 * Route: /mall/b2f-inspection/:batchId
 */
...
import { fetchB2FBatchDetail, fetchB2FItemHistory } from '@/api/b2fInspection'
import type {
  B2FInspectionBatchDetail, B2FInspectionItem, B2FInspectionResultStatus,
  B2FInspectionItemHistory, B2FInspectionDailySummary,
...
function ItemHistoryDrawer({
  ...
  const selectedDayData: B2FInspectionDailySummary | undefined = selectedDay
    ? daily_summary.find(ds => ds.inspection_date === selectedDay)
    : undefined

  function DayCell({ ds }: { ds: B2FInspectionDailySummary }) {
...
export default function B2FInspectionDetailPage() {
  const { batchId } = useParams<{ batchId: string }>()
  const navigate    = useNavigate()

  const [detail, setDetail]   = useState<B2FInspectionBatchDetail | null>(null)
  ...
  const [historyData, setHistoryData]       = useState<B2FInspectionItemHistory | null>(null)

  const loadDetail = useCallback(async () => {
    if (!batchId) return
    setLoading(true)
    try { setDetail(await fetchB2FBatchDetail(batchId)) }
  ...
  async function openHistory(itemName: string) {
    ...
    try { setHistoryData(await fetchB2FItemHistory(itemName, 30)) }
  ...
  const columns: ColumnsType<B2FInspectionItem> = [
    ...
    { title: '巡檢結果', dataIndex: 'result_status', width: 100, align: 'center',
      render: (v: B2FInspectionResultStatus, rec) => {
  ...
  return (
    <div style={{ padding: '0 0 32px' }}>
      ...
      <Card bodyStyle={{ padding: 0 }}>
        ...
        <Table<B2FInspectionItem> loading={loading} dataSource={filteredItems} rowKey="ragic_id"
