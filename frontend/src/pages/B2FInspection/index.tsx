...
import { fetchB2FStats, fetchB2FBatches, syncB2FFromRagic } from '@/api/b2fInspection'
import type {
  B2FInspectionStats, B2FInspectionBatchListItem, B2FInspectionItem,
...
function deriveBatchStatus(kpi: B2FInspectionBatchListItem['kpi']): { label: string; color: string } {
...
export default function B2FInspectionPage() {
  const navigate = useNavigate()
  const [activeTab, setActiveTab]   = useState('dashboard')
  const [stats, setStats]           = useState<B2FInspectionStats | null>(null)
  const [batches, setBatches]       = useState<B2FInspectionBatchListItem[]>([])
  ...
  const loadDashboard = useCallback(async () => {
    setLoading(true)
    try { setStats(await fetchB2FStats()) }
  ...
  const loadBatches = useCallback(async () => {
    setLoading(true)
    try { setBatches(await fetchB2FBatches({ year_month: yearMonth })) }
    catch { message.error('載入巡檢紀錄失敗') }
    finally { setLoading(false) }
  }, [yearMonth])
  ...
  const trendData = (stats?.abnormal_trend ?? []).map(t => ({
    date: t.date.slice(5), 異常數量: t.abnormal_count,
  }))
  ...
  const DashboardTab = (
    <div>
      <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
        {[
          { title: '已巡檢（本次）', value: kpi ? kpi.total - kpi.unchecked : 0, suffix: `/${kpi?.total ?? 0} 項`, icon: <SafetyOutlined />, color: '#1B3A5C' },
          { title: `正常（${kpi?.normal_rate ?? 0}%）`,  value: kpi?.normal   ?? 0, suffix: '項', icon: <CheckCircleOutlined />,     color: '#52C41A' },
          { title: '異常',    value: kpi?.abnormal ?? 0, suffix: '項', icon: <WarningOutlined />,           color: '#FF4D4F' },
          { title: '待處理',  value: kpi?.pending  ?? 0, suffix: '項', icon: <ExclamationCircleOutlined />, color: '#FAAD14' },
        ].map(card => (
      ...
      {kpi && kpi.total > 0 && (
        <Card size="small" style={{ marginBottom: 16 }}>
          <Row align="middle" gutter={16}>
            ...
            <Col flex="auto">
              <Progress percent={kpi.completion_rate}
        ...
        <Col xs={24} lg={10}>
          <Card title="本次巡檢狀態分布" size="small" style={{ height: 300 }}>
            {pieData.length > 0 ? (
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie data={pieData} dataKey="count" nameKey="label"
        ...
        <Col xs={24} lg={12}>
          <Card title={<><WarningOutlined style={{ color: '#FF4D4F' }} /> 本次異常項目</>} size="small">
            {(stats?.recent_abnormal ?? []).length === 0
              ...
              : <Table<B2FInspectionItem> dataSource={stats?.recent_abnormal ?? []} rowKey="ragic_id"
                  size="small" pagination={false} columns={[
                    { title: '設備項目', dataIndex: 'item_name', ellipsis: true },
                    { title: '狀態', dataIndex: 'result_status', width: 72,
                      render: (v) => { const c = STATUS_CFG[v] ?? { tagColor: 'default', label: v }; return <Tag color={c.tagColor}>{c.label}</Tag> } },
                    { title: '', width: 44,
                      render: (_, row) => <Button type="link" size="small" icon={<RightOutlined />}
                        onClick={() => navigate(`/mall/b2f-inspection/${row.batch_ragic_id}`)} /> },
                  ]} />
            }
          </Card>
        </Col>
        <Col xs={24} lg={12}>
          <Card title={<><ClockCircleOutlined style={{ color: '#FAAD14' }} /> 待處理項目</>} size="small">
            {(stats?.recent_pending ?? []).length === 0
              ...
              : <Table<B2FInspectionItem> dataSource={stats?.recent_pending ?? []} rowKey="ragic_id"
                  size="small" pagination={false} columns={[
                    { title: '設備項目', dataIndex: 'item_name', ellipsis: true },
          ...
          <Row align="middle" justify="space-between">
            <Col>
              <Space>
                ...
                {stats.latest_batch.start_time    && <Text type="secondary">{stats.latest_batch.start_time}</Text>}
                {stats.latest_batch.inspector_name && <Text type="secondary">巡檢人員：{stats.latest_batch.inspector_name}</Text>}
                {stats.latest_batch.work_hours     && <Tag color="blue">{stats.latest_batch.work_hours}</Tag>}
              </Space>
            </Col>
            <Col>
              <Button type="primary" icon={<RightOutlined />} style={{ background: '#1B3A5C' }}
                onClick={() => navigate(`/mall/b2f-inspection/${stats.latest_batch!.ragic_id}`)}>
                查看最新明細
              </Button>
            </Col>
          </Row>
  ...
  const batchColumns: ColumnsType<B2FInspectionBatchListItem> = [
    { title: '巡檢日期', dataIndex: ['batch', 'inspection_date'], width: 110,
      sorter: (a, b) => a.batch.inspection_date.localeCompare(b.batch.inspection_date), defaultSortOrder: 'descend' },
    { title: '開始時間', dataIndex: ['batch', 'start_time'], width: 150,
      render: (v) => <Text type="secondary" style={{ fontSize: 12 }}>{v || '—'}</Text> },
    { title: '結束時間', dataIndex: ['batch', 'end_time'], width: 150,
      render: (v) => <Text type="secondary" style={{ fontSize: 12 }}>{v || '—'}</Text> },
    { title: '工時', dataIndex: ['batch', 'work_hours'], width: 90,
    ...
    { title: '巡檢人員', dataIndex: ['batch', 'inspector_name'], width: 100,
      render: (v, row) => (
        <Button type="link" style={{ padding: 0, fontWeight: 600 }}
          onClick={() => navigate(`/mall/b2f-inspection/${row.batch.ragic_id}`)}>
          {v || row.batch.ragic_id}
        </Button>
      ) },
    { title: '狀態', width: 90,
      render: (_, row) => { const s = deriveBatchStatus(row.kpi); return <Tag color={s.color}>{s.label}</Tag> } },
        ...
        return (
          <div>
            <Progress percent={completion_rate} size="small"
              strokeColor={{ from: '#FAAD14', to: '#52C41A' }} format={() => `${completion_rate}%`} />
            <Text type="secondary" style={{ fontSize: 11 }}>{checked} / {total} 已巡檢</Text>
          </div>
        )
      } },
  ...
  const ListTab = (
    <div>
      <Row gutter={8} style={{ marginBottom: 16 }}>
        <Col>
          <DatePicker picker="month" value={dayjs(yearMonth, 'YYYY/MM')} format="YYYY/MM" allowClear={false}
      ...
      <Table<B2FInspectionBatchListItem>
  ...
  return (
    <div style={{ padding: '0 4px' }}>
      <Breadcrumb style={{ marginBottom: 12 }} items={[
        ...
        { title: NAV_PAGE.b2fInspection },
      ]} />
      <Row align="middle" justify="space-between" style={{ marginBottom: 16 }}>
        <Col>
          <Title level={4} style={{ margin: 0, color: '#1B3A5C' }}>
            <SafetyOutlined /> {NAV_PAGE.b2fInspection}
          </Title>
        </Col>
        <Col>
