/**
 * 整棟工務每日巡檢 - B1F  批次明細頁（唯讀）【寬表格 Pivot 架構】
 * Route: /mall/b1f-inspection/:batchId
 */
...
import { fetchB1FBatchDetail, fetchB1FItemHistory } from '@/api/b1fInspection'
import type {
  B1FInspectionBatchDetail, B1FInspectionItem, B1FInspectionResultStatus,
  B1FInspectionItemHistory, B1FInspectionDailySummary,
...
function ItemHistoryDrawer({
  ...
  open: boolean
  onClose: () => void
  historyData: B1FInspectionItemHistory | null
  loading: boolean
  ragicId?: string
}) {
  ...
  const { item_name, daily_summary, stats } = historyData ?? {
    item_name: '', daily_summary: [],
    stats: { total_days: 30, normal_days: 0, abnormal_days: 0 },
  }

  const selectedDayData: B1FInspectionDailySummary | undefined = selectedDay
    ? daily_summary.find(ds => ds.inspection_date === selectedDay)
    : undefined

  function DayCell({ ds }: { ds: B1FInspectionDailySummary }) {
    ...
    return (
      <Tooltip title={!ds.has_record
        ...
        <div
          ...
          style={{
            background: bg, border: isSelected ? '2px solid #1B3A5C' : `1px solid ${borderColor}`,
            borderRadius: 6, padding: '5px 3px', textAlign: 'center', minWidth: 52,
            cursor: ds.has_record ? 'pointer' : 'default', transition: 'all .15s',
  ...
  return (
    <Drawer
        ...
        <Space>
          ...
          {ragicId && (
            <Tooltip title="在 Ragic 查看此批次原始紀錄">
              <a
                href={`https://ap12.ragic.com/soutlet001/full-building-inspection/4/${ragicId}`}
                target="_blank"
      ...
      <Row gutter={12} style={{ marginBottom: 16 }}>
        <Col span={8}>
          <Card size="small">
            <Statistic title="近30日正常天數" value={`${stats.normal_days} / ${stats.total_days}`}
              prefix={<CheckCircleOutlined />} valueStyle={{ color: '#52C41A', fontSize: 18 }} />
          </Card>
        </Col>
        <Col span={8}>
          <Card size="small">
            <Statistic title="正常率"
              value={stats.total_days > 0 ? Math.round(stats.normal_days / stats.total_days * 100) : 0}
              suffix="%"
              valueStyle={{ color: stats.normal_days / stats.total_days >= 0.9 ? '#52C41A'
                : stats.normal_days / stats.total_days >= 0.7 ? '#FAAD14' : '#FF4D4F', fontSize: 18 }} />
          </Card>
        </Col>
        <Col span={8}>
          <Card size="small">
            <Statistic title="異常天數" value={stats.abnormal_days} prefix={<WarningOutlined />}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20 }}>
        {daily_summary.map(ds => <DayCell key={ds.inspection_date} ds={ds} />)}
      </div>

      {selectedDayData && (
        <>
          <Divider orientation="left" orientationMargin={0}>
            <Text type="secondary" style={{ fontSize: 12 }}>{selectedDayData.inspection_date} 執行記錄</Text>
          </Divider>
          <Card size="small" style={{ background: selectedDayData.result_status === 'normal' ? '#f6ffed' : '#fff7e6' }}>
            <Row gutter={[12, 8]}>
              <Col span={12}><Text type="secondary">巡檢人員：</Text><Text>{selectedDayData.inspector_name || '—'}</Text></Col>
              <Col span={12}><Text type="secondary">開始時間：</Text><Text>{selectedDayData.start_time || '—'}</Text></Col>
              <Col span={24}>
...
export default function B1FInspectionDetailPage() {
  const { batchId } = useParams<{ batchId: string }>()
  const navigate    = useNavigate()

  const [detail, setDetail]   = useState<B1FInspectionBatchDetail | null>(null)
  ...
  const [historyData, setHistoryData]       = useState<B1FInspectionItemHistory | null>(null)

  const loadDetail = useCallback(async () => {
    if (!batchId) return
    setLoading(true)
    try { setDetail(await fetchB1FBatchDetail(batchId)) }
    catch { message.error('載入巡檢資料失敗') }
  ...
  async function openHistory(itemName: string) {
    setHistoryOpen(true); setHistoryLoading(true); setHistoryData(null)
    try { setHistoryData(await fetchB1FItemHistory(itemName, 30)) }
    catch { message.error('載入歷史記錄失敗') }
  ...
  const filteredItems = (detail?.items ?? []).filter(item => {
    ...
    } else if (activeTab !== 'all' && item.result_status !== activeTab) return false
  ...
  const columns: ColumnsType<B1FInspectionItem> = [
    { title: '項次', dataIndex: 'seq_no', width: 60, align: 'center',
      render: (v) => <Text type="secondary">{v}</Text> },
    { title: '巡檢項目', dataIndex: 'item_name',
      render: (v: string) => (
        <Button type="link" size="small" style={{ padding: 0, height: 'auto', fontWeight: 600, textAlign: 'left' }}
          onClick={() => openHistory(v)}>
          {v}<HistoryOutlined style={{ marginLeft: 4, fontSize: 11, color: '#4BA8E8' }} />
        </Button>
      ) },
    { title: '巡檢結果', dataIndex: 'result_status', width: 100, align: 'center',
      render: (v: B1FInspectionResultStatus, rec) => {
        const cfg = STATUS_CFG[v] ?? { label: v, tagColor: 'default' }
        return <Tooltip title={rec.result_raw || cfg.label}><Tag color={cfg.tagColor}>{cfg.label}</Tag></Tooltip>
      } },
    { title: '原始值', dataIndex: 'result_raw', width: 100,
  ...
  return (
    <div style={{ padding: '0 0 32px' }}>
      <Breadcrumb style={{ marginBottom: 16 }} items={[
        { title: '商場管理' },
        { title: <a onClick={() => navigate('/mall/dashboard?tab=b1f')}>工務巡檢 - B1F</a> },
        { title: batch?.inspection_date ?? batchId },
      ]} />

      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <Space>
          <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/mall/dashboard?tab=b1f')}>返回列表</Button>
        ...
        <Button icon={<ReloadOutlined />} onClick={loadDetail} loading={loading}>重新整理</Button>
      </div>

      {batch && (
        <Card size="small" style={{ marginBottom: 16, background: '#fafcff' }}>
          <Row gutter={[16, 4]}>
            <Col xs={12} sm={6}><Space size={4}><ClockCircleOutlined style={{ color: '#4BA8E8' }} /><Text type="secondary">開始：</Text><Text>{batch.start_time || '—'}</Text></Space></Col>
            <Col xs={12} sm={6}><Space size={4}><ClockCircleOutlined style={{ color: '#4BA8E8' }} /><Text type="secondary">結束：</Text><Text>{batch.end_time || '—'}</Text></Space></Col>
            <Col xs={12} sm={6}><Space size={4}><Text type="secondary">工時：</Text><Text>{batch.work_hours || '—'}</Text></Space></Col>
            <Col xs={12} sm={6}><Space size={4}><Text type="secondary">巡檢人員：</Text><Text strong>{batch.inspector_name || '—'}</Text></Space></Col>
          </Row>
        </Card>
      )}
        ...
        {kpiCards.map(c => (
          <Col key={c.label} xs={12} sm={8} md={4}>
            <Card size="small" style={{ textAlign: 'center' }}>
              <Statistic title={c.label} value={c.value} valueStyle={{ color: c.color, fontSize: 22 }} />
            </Card>
          </Col>
        ))}
        {kpi && (
            ...
            <Card size="small">
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ flex: 1, height: 12, background: '#f0f0f0', borderRadius: 6, overflow: 'hidden' }}>
                  <div style={{ width: `${kpi.completion_rate}%`, height: '100%',
                    background: kpi.completion_rate >= 90 ? '#52C41A' : kpi.completion_rate >= 60 ? '#FAAD14' : '#FF4D4F',
                    borderRadius: 6, transition: 'width 0.5s' }} />
                </div>
                <Text strong style={{ minWidth: 48, textAlign: 'right' }}>{kpi.completion_rate}%</Text>
              </div>
            </Card>
          </Col>
        )}
      ...
      <Card size="small" style={{ marginBottom: 16, borderColor: isAdvanced ? '#4BA8E8' : undefined }}>
        <Space wrap>
          <Input placeholder="搜尋巡檢項目" prefix={<SearchOutlined />} value={searchText}
        ...
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px dashed #e0e0e0',
          ...
          <Text type="secondary" style={{ fontSize: 12, whiteSpace: 'nowrap' }}>
            <FilterOutlined style={{ marginRight: 4 }} />狀態複選：
          </Text>
          <Select mode="multiple" placeholder="選擇狀態（空白 = 使用 Tab）"
            value={statusFilters} onChange={setStatusFilters} allowClear
            style={{ minWidth: 280, flex: 1 }} maxTagCount="responsive"
            options={STATUS_TABS.filter(t => t.key !== 'all').map(t => ({
              value: t.key,
              label: (
                <span>
                  <Tag color={STATUS_CFG[t.key]?.tagColor} style={{ margin: '0 4px 0 0', fontSize: 11 }}>{t.label}</Tag>
                  <Text type="secondary" style={{ fontSize: 11 }}>({tabCounts[t.key] ?? 0})</Text>
                </span>
              ),
            }))} />
          {isAdvanced && <Button size="small" onClick={() => setStatusFilters([])}>清除複選</Button>}
        </div>
      </Card>
      ...
      <Card bodyStyle={{ padding: 0 }}>
        <div style={{ borderBottom: '1px solid #f0f0f0', padding: '0 16px', display: 'flex', gap: 0, overflowX: 'auto' }}>
          {STATUS_TABS.map(t => {
            ...
            return (
              <div key={t.key} onClick={() => { setStatusFilters([]); setActiveTab(t.key) }}
                style={{ padding: '12px 16px', fontSize: 14, cursor: 'pointer', whiteSpace: 'nowrap',
                  borderBottom: isActive ? '2px solid #1B3A5C' : '2px solid transparent',
                  color: isActive ? '#1B3A5C' : '#595959', fontWeight: isActive ? 600 : 400, transition: 'all .2s' }}>
                {t.label}
                {tabCounts[t.key] !== undefined && (
                  <Tag style={{ marginLeft: 4, fontSize: 11 }}
                    color={tabCounts[t.key] > 0 && ['abnormal', 'pending'].includes(t.key) ? 'error' : undefined}>
                    {tabCounts[t.key] ?? 0}
                  </Tag>
                )}
              </div>
          ...
          {statusFilters.length > 0 && (
            <div style={{ padding: '12px 16px', fontSize: 14, cursor: 'default', whiteSpace: 'nowrap',
              borderBottom: '2px solid #4BA8E8', color: '#4BA8E8', fontWeight: 600 }}>複選篩選中</div>
          )}
        </div>
        <Table<B1FInspectionItem> loading={loading} dataSource={filteredItems} rowKey="ragic_id"
          columns={columns} size="small"
          pagination={{ pageSize: 50, showTotal: n => `共 ${n} 筆` }}
          onRow={rec => ({ style: { background: STATUS_CFG[rec.result_status]?.rowBg } })} />
      </Card>
