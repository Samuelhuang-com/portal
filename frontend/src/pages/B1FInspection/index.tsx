...
function deriveBatchStatus(kpi: B1FInspectionBatchListItem['kpi']): { label: string; color: string } {
...
export default function B1FInspectionPage() {
  const navigate = useNavigate()
  const [activeTab, setActiveTab]   = useState('dashboard')
  const [stats, setStats]           = useState<B1FInspectionStats | null>(null)
  ...
  const loadDashboard = useCallback(async () => {
    setLoading(true)
    try { setStats(await fetchB1FStats()) }
  ...
  const loadBatches = useCallback(async () => {
    setLoading(true)
    try { setBatches(await fetchB1FBatches({ year_month: yearMonth })) }
        ...
        <Col xs={24} lg={12}>
          <Card title={<><WarningOutlined style={{ color: '#FF4D4F' }} /> 本次異常項目</>} size="small">
            {(stats?.recent_abnormal ?? []).length === 0
              ...
              : <Table<B1FInspectionItem> dataSource={stats?.recent_abnormal ?? []} rowKey="ragic_id"
                  size="small" pagination={false} columns={[
                    ...
                    { title: '', width: 44,
                      render: (_, row) => <Button type="link" size="small" icon={<RightOutlined />}
                        onClick={() => navigate(`/mall/b1f-inspection/${row.batch_ragic_id}`)} /> },
                  ]} />
            }
          </Card>
        </Col>
        <Col xs={24} lg={12}>
          <Card title={<><ClockCircleOutlined style={{ color: '#FAAD14' }} /> 待處理項目</>} size="small">
            {(stats?.recent_pending ?? []).length === 0
              ...
              : <Table<B1FInspectionItem> dataSource={stats?.recent_pending ?? []} rowKey="ragic_id"
                  size="small" pagination={false} columns={[
                    { title: '設備項目', dataIndex: 'item_name', ellipsis: true },
          ...
          <Row align="middle" justify="space-between">
            ...
            <Col>
              <Button type="primary" icon={<RightOutlined />} style={{ background: '#1B3A5C' }}
                onClick={() => navigate(`/mall/b1f-inspection/${stats.latest_batch!.ragic_id}`)}>
                查看最新明細
              </Button>
            </Col>
          </Row>
  ...
  const batchColumns: ColumnsType<B1FInspectionBatchListItem> = [
    ...
    { title: '巡檢人員', dataIndex: ['batch', 'inspector_name'], width: 100,
      render: (v, row) => (
        <Button type="link" style={{ padding: 0, fontWeight: 600 }}
          onClick={() => navigate(`/mall/b1f-inspection/${row.batch.ragic_id}`)}>
  ...
  const ListTab = (
    <div>
      ...
      <Table<B1FInspectionBatchListItem>
