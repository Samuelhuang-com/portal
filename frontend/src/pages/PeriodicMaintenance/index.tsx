...
import {
  fetchPMStats, fetchPMBatches, fetchPMPeriodStats, fetchPMYearMatrix,
  fetchPMCalendar, fetchPMBatchDetail, fetchPMMatrixItems, fetchPMCatalog,
  generatePMSchedule, fetchPMSchedule, fetchPMScheduleKPI,
  fetchPMOverdueSchedule, fetchPMAnnualMatrix, updatePMSchedule,
} from '@/api/periodicMaintenance'
...
import type {
  PMStats, PMBatchListItem, PMItem, PMItemStatus, PMPeriodStats,
  PMIncompleteItem, PMSubPeriodBreakdown, PMYearMatrix, PMYearMatrixMonth,
  PMScheduleItem, PMScheduleKPI, PMScheduleGenerateResult,
  PMScheduleAnnualMatrix, PMMatrixCellStatus,
...
// ── Dashboard KPI 卡片明細 Drawer 用格式化（2026-07-14 新增，比照 mall_pm 同名函式）───
...
export default function PeriodicMaintenancePage() {
  ...
  // ── Dashboard KPI 卡片明細 Drawer state（2026-07-14 新增，比照 mall_pm 同型 Drawer）──
  ...
  // KPI 明細內「保養時間」次層 Drawer state（共用 PMItemDetailDrawer）
  ...
  const [schedItems,      setSchedItems]      = useState<PMScheduleItem[]>([])
  const [schedKpi,        setSchedKpi]        = useState<PMScheduleKPI | null>(null)
  ...
  const [overdueItems,    setOverdueItems]    = useState<PMScheduleItem[]>([])
  ...
  const [schedDrawerItem, setSchedDrawerItem] = useState<PMScheduleItem | null>(null)
  ...
  const loadSchedule = useCallback(async () => {
    ...
    try {
      const [listRes, kpiRes] = await Promise.all([
        fetchPMSchedule(schedYearMonth, schedCatFilter, schedStatusFilter),
        fetchPMScheduleKPI(schedYearMonth),
      ])
  ...
  const loadOverdue = useCallback(async () => {
    ...
    try {
      const res = await fetchPMOverdueSchedule()
  ...
  const handleGenerateSchedule = useCallback(async () => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const res: PMScheduleGenerateResult = await generatePMSchedule(schedYear, schedMonth)
          message.success(
  ...
  const openSchedDrawer = useCallback((item: PMScheduleItem) => {
  ...
  const handleSchedUpdate = useCallback(async () => {
    ...
    try {
      ...
      await updatePMSchedule(schedDrawerItem.id, values)
      message.success('已更新')
  ...
  // ── Dashboard KPI 卡片明細 Drawer（2026-07-14 新增，比照 mall_pm openKpiDrawer）───────
  ...
  const openKpiDrawer = useCallback(async (metric: KpiCardMetric, title: string) => {
    ...
    try {
      const detail = await fetchPMBatchDetail(stats.current_batch.ragic_id)
      let filtered: PMItem[]
      switch (metric) {
        ...
        case 'overdue':
          ...
          break
        case 'abnormal':
          // 比照 KPI 卡片數字口徑改為只算本月項目，避免點進來的清單筆數跟卡片數字對不起來
  ...
  const schedColumns: ColumnsType<PMScheduleItem> = [
    {
      title: '類別', dataIndex: 'category', width: 80,
    ...
    {
      title: '狀態', width: 90,
      render: (_: unknown, row: PMScheduleItem) => {
  ...
  // 逾期清單欄位
  const overdueColumns: ColumnsType<PMScheduleItem & { overdue_days?: number }> = [
  ...
  const ScheduleTab = (
    <div>
      ...
      {overdueVisible && (
        <Table<PMScheduleItem & { overdue_days?: number }>
          dataSource={overdueItems as (PMScheduleItem & { overdue_days?: number })[]}
          rowKey="id"
        ...
        schedDrawerItem && (
          <Space>
            ...
            {schedDrawerItem.item_ragic_id && (
              <Tooltip title="在 Ragic 查看原始表單">
                <a
                  href={`https://ap12.ragic.com/soutlet001/periodic-maintenance/11/${schedDrawerItem.item_ragic_id}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ color: '#4BA8E8', fontSize: 16, lineHeight: 1 }}
                >
  ...
  const [annualMatrix,  setAnnualMatrix]  = useState<PMScheduleAnnualMatrix | null>(null)
  const [annualLoading, setAnnualLoading] = useState(false)
  const [annualDrawerOpen, setAnnualDrawerOpen] = useState(false)
  const [annualDrawerCell, setAnnualDrawerCell] = useState<{
    scheduleId: number | null; status: PMMatrixCellStatus; month: number; row: PMScheduleAnnualMatrix['rows'][0]
  } | null>(null)
  const [annualCellDetail, setAnnualCellDetail] = useState<PMScheduleItem | null>(null)
  const [annualCellLoading, setAnnualCellLoading] = useState(false)

  const loadAnnualMatrix = useCallback(async () => {
    setAnnualLoading(true)
    try {
      const res = await fetchPMAnnualMatrix(annualYear, annualCat)
      setAnnualMatrix(res)
    } catch {
  ...
  const openAnnualDrawer = useCallback(async (
    ...
    month: number,
    row: PMScheduleAnnualMatrix['rows'][0],
  ) => {
    ...
    if (scheduleId) {
      ...
      try {
        const res = await fetchPMSchedule(
      ...
      title={annualDrawerCell && (
        <Space>
          ...
          {annualDrawerCell.row.item_ragic_id && (
            <Tooltip title="在 Ragic 查看原始表單">
              <a
                href={`https://ap12.ragic.com/soutlet001/periodic-maintenance/11/${annualDrawerCell.row.item_ragic_id}`}
                target="_blank"
                rel="noopener noreferrer"
                style={{ color: '#4BA8E8', fontSize: 16, lineHeight: 1 }}
              >
  ...
  return (
    ...
    {/* ── Dashboard KPI 卡片明細 Drawer（2026-07-14 新增，比照 mall_pm 同型 Drawer）──── */}
    <Drawer
      open={kpiDrawerOpen}
      width={760}
