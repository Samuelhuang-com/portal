/**
 * 週期保養表 — 批次明細頁（唯讀，資料來源全部為 Ragic）
 * Route: /hotel/periodic-maintenance/:batchId
 *
 * 設計原則：Portal 只呈現 Ragic 資料，不提供任何編輯功能。
 * 狀態由 Ragic 欄位推導：
 *   保養時間迄有值 → 已完成
 *   保養時間啟有值（無迄）→ 進行中
 *   排定日期有值且已過 → 逾期
...
import { fetchPMBatchDetail, fetchPMTaskHistory } from '@/api/periodicMaintenance'
import type {
...
// ══════════════════════════════════════════════════════════════════════════════
// ItemHistoryDrawer — 保養項目近 12 個月執行歷史（唯讀）
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
// Main Component — 唯讀明細頁
// ══════════════════════════════════════════════════════════════════════════════
export default function PeriodicMaintenanceDetailPage() {
  ...
  const loadDetail = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const data = await fetchPMBatchDetail(batchId)
      setDetail(data)
    } catch {
  ...
  async function openHistory(taskName: string) {
    ...
    setHistoryData(null)
    try {
      const data = await fetchPMTaskHistory(taskName)
      setHistoryData(data)
    } catch {
  ...
  return (
    <div style={{ padding: '0 0 32px' }}>
      ...
      <Breadcrumb
        ...
        items={[
          { title: '飯店管理' },
          { title: <a onClick={() => navigate('/hotel/periodic-maintenance')}>週期保養表</a> },
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <Space>
          <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/hotel/periodic-maintenance')}>
            返回列表
          </Button>
