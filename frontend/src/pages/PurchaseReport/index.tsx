...
export default function PurchaseReportPage() {
  ...
  // ── 總表專用篩選 ──────────────────────────────────────────────────────────────
  const [sourceTypeFilter, setSourceTypeFilter]         = useState<'purchase' | 'claim' | undefined>(undefined)
  const [combinedSearchInput, setCombinedSearchInput]   = useState<string>('')
  const [combinedSearchKeyword, setCombinedSearchKeyword] = useState<string>('')
  ...
  const [combinedOrders, setCombinedOrders]         = useState<CombinedOrderRow[]>([])
  const [combinedOrdersTotal, setCombinedOrdersTotal] = useState(0)
  const [combinedOrdersPage, setCombinedOrdersPage] = useState(1)
  ...
  const loadCombinedOrders = useCallback((page = 1) => {
    setCombinedOrdersLoading(true)
    setCombinedOrdersPage(page)
    getCombinedOrders({
      year_month: yearMonthFrom ? undefined : yearMonth,
      year_month_from: yearMonthFrom, year_month_to: yearMonthTo,
      department: deptFilter,
      source_type: sourceTypeFilter,
      keyword: combinedSearchKeyword || undefined,
      page, per_page: 20,
    })
      .then((r) => { setCombinedOrders(r.data.items ?? []); setCombinedOrdersTotal(r.data.total ?? 0) })
      .finally(() => setCombinedOrdersLoading(false))
  }, [yearMonth, yearMonthFrom, yearMonthTo, deptFilter, sourceTypeFilter, combinedSearchKeyword])
  ...
  useEffect(() => {
    ...
    else if (activeTab === 'combined') { loadCombinedOrders(1) }
    else if (activeTab === 'dept') {
      loadCombinedDeptStats()
      loadClaimDeptStats()
  ...
  const purchaseOrderColumns = [
    ...
    { title: '最後更新', dataIndex: 'last_updated_at', key: 'last_updated_at', width: 150,
  ...
  const combinedColumns = [
    { title: '類型', dataIndex: 'source_type', key: 'source_type', width: 68, fixed: 'left' as const,
      render: (v: string) => v === 'purchase'
        ? <Tag color="blue">請購</Tag>
        : <Tag color="orange">請款</Tag> },
    { title: '核准日期', dataIndex: 'approved_date', key: 'approved_date', width: 100, fixed: 'left' as const,
      render: (v: string | null) => v ?? '-' },
    { title: '部門', dataIndex: 'department_display', key: 'dept', width: 80,
      render: (v: string, r: CombinedOrderRow) => (
        <Tag color={r.source_type === 'purchase' ? 'blue' : 'orange'}>{v}</Tag>
      ) },
    { title: '單號', dataIndex: 'doc_no', key: 'doc_no', width: 175, ellipsis: true,
      render: (v: string) => <span style={{ fontFamily: 'monospace', fontSize: 12 }}>{v || '-'}</span> },
    { title: '會科', dataIndex: 'account_label', key: 'account_label', width: 120, ellipsis: true,
      render: (v: string | null) => v || <span style={{ color: '#aaa' }}>—</span> },
    { title: '申請人', dataIndex: 'applicant', key: 'applicant', width: 80 },
    { title: '說明/事由', dataIndex: 'description', key: 'description', ellipsis: true,
      render: (v: string | null) => v || <span style={{ color: '#aaa' }}>—</span> },
    { title: '未稅金額', dataIndex: 'amount', key: 'amount', width: 105, align: 'right' as const,
      render: (v: number | null, r: CombinedOrderRow) =>
        r.source_type === 'purchase'
          ? <span style={{ color: '#1B3A5C', fontWeight: 600 }}>{fmt(v)}</span>
          : <span style={{ color: '#aaa' }}>—</span> },
    { title: '應付金額', dataIndex: 'payable_amount', key: 'payable_amount', width: 105, align: 'right' as const,
      render: (v: number | null, r: CombinedOrderRow) =>
        r.source_type === 'claim'
          ? <span style={{ color: '#d46b08', fontWeight: 600 }}>{fmt(v)}</span>
          : <span style={{ color: '#aaa' }}>—</span> },
    { title: '狀態', dataIndex: 'status', key: 'status', width: 80, render: statusTag },
  ]
  ...
  const isCombinedTab  = activeTab === 'combined'        || activeTab === 'dept'
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        <Title level={4} style={{ margin: 0 }}>
          {isPurchaseTab ? '核准請購單月報表' : isClaimTab ? '核准請款單月報表' : '請購 + 請款整合總表'}
        </Title>
        <Space wrap style={{ justifyContent: 'flex-end' }}>
          ...
          {isCombinedTab && (
            <>
              <Select placeholder="全部類型" allowClear style={{ width: 110 }}
                value={sourceTypeFilter} onChange={setSourceTypeFilter}
                options={[{ label: '請購', value: 'purchase' }, { label: '請款', value: 'claim' }]} />
              {activeTab === 'combined' && (
                <Input.Search
                  placeholder="說明/單號/申請人" allowClear style={{ width: 220 }}
                  value={combinedSearchInput}
                  onChange={(e) => { setCombinedSearchInput(e.target.value); if (!e.target.value) setCombinedSearchKeyword('') }}
                  onSearch={(v) => setCombinedSearchKeyword(v.trim())}
                />
              )}
            </>
          )}
      ...
      <Card bodyStyle={{ padding: 0 }}>
        <Tabs
          ...
          items={[
            ...
            {
              key: 'combined',
              label: <span><ApartmentOutlined style={{ marginRight: 4 }} />總表（{combinedOrdersTotal} 筆）</span>,
              children: (
                <div style={{ padding: '0 16px 16px' }}>
                  <Table<CombinedOrderRow>
                    dataSource={combinedOrders}
                    rowKey={(r) => `${r.source_type}-${r.id}`}
                    columns={combinedColumns} loading={combinedOrdersLoading} size="small" scroll={{ x: 1100 }}
                    onRow={(r) => ({
                      onClick: () => r.source_type === 'purchase' ? openPurchaseDrawer(r.id) : openClaimDrawer(r.id),
                      style: { cursor: 'pointer' },
                    })}
                    rowClassName={(r) => r.source_type === 'purchase' ? 'combined-row-purchase' : 'combined-row-claim'}
                    pagination={{
                      total: combinedOrdersTotal, current: combinedOrdersPage, pageSize: 20,
                      showSizeChanger: false,
                      showTotal: (t) => `共 ${t} 筆（請購 ${combinedSummary?.purchase.order_count ?? 0} + 請款 ${combinedSummary?.claim.order_count ?? 0}）`,
                      onChange: (p) => loadCombinedOrders(p),
                    }}
                  />
                  <style>{`
                    .combined-row-purchase td { border-left: 3px solid #4BA8E8 !important; }
                    .combined-row-claim td { border-left: 3px solid #ffa940 !important; }
                    .combined-row-purchase:hover td { background: #f0f7ff !important; }
                    .combined-row-claim:hover td { background: #fff7e0 !important; }
                  `}</style>
                </div>
              ),
            },
