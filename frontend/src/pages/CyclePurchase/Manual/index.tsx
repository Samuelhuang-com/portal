/**
 * 週期採購（週採）— 使用手冊（/cycle-purchase/manual）
 *
 ...
 *
 * ⚠️ 刻意**不重用** `pages/Realtime/Manual` 或 `pages/Opera/Manual` ——
 *    三個模組彼此獨立，重用會讓其中一邊的改動波及另外兩邊。
 *    版面骨架與 Realtime 版相同（複製而非引用），是刻意的重複；
 *    Opera 版多出來的「即時抓資料庫數字」「章節實作狀態」週採用不到。
 *
 * 權限：與 Samuel 確認開放給**所有週採使用者**（MainLayout 用 permissionKeys
 *    OR 涵蓋全部 8 個週採權限）。手冊是教人怎麼用的，卡權限會讓最需要它的
 *    填單人看不到，所以不另立新的 permission key、路由也不加 PermissionGuard
 *    （與週採其他路由一致，存取控制在選單與後端端點）。
 */
        ...
        <Col xs={24} md={18}>
          <Card size="small" style={{ marginBottom: 16 }}>
            <Input
              ...
              placeholder="搜尋手冊內容，例如「關閉」「分批驗收」「分攤」"
              value={keyword}
          ...
          <Text type="secondary" style={{ fontSize: 12 }}>
            欄位層級的完整規格：
            <Text code style={{ fontSize: 11 }}>ragic-cycle-purchase/docs/欄位規格.md</Text>
            　規劃背景：
            <Text code style={{ fontSize: 11 }}>ragic-cycle-purchase/docs/週期採購_Portal規劃評估_v1.1.md</Text>
          </Text>
        </Col>
      </Row>
    </div>
