/**
 * 【商場班表】此檔為 pages/Schedule/index.tsx 的對稱複製版。
 * 差異只有 API 來源（@/api/mallSchedule）與路由前綴（/mall/schedule）。
 * 修改任一邊的版面或行為時，請同步檢視另一邊。
 *
 * 班表總覽頁
 * 路由：/schedule
 * 預設呈現兩個 Tab：
...
import {
  mallFetchSchedules, mallFetchScheduleTable, mallFetchDetailList,
  mallFetchStaff, mallFetchShifts, mallFetchDepartments, mallFetchStats,
  mallEditDetail, mallDeleteDetail, mallAddDetail, mallDeleteSchedule,
} from '@/api/mallSchedule'
import type {
...
export default function ScheduleOverviewPage() {
  ...
  const loadResources = useCallback(async () => {
    const [sl, sh, dp] = await Promise.all([
      mallFetchStaff({ is_active: true }),
      mallFetchShifts(),
      mallFetchDepartments(),
    ])
    setStaffList(sl)
    setShiftList(sh)
    setDeptList(dp)
  ...
  const loadSchedules = useCallback(async () => {
    const list = await mallFetchSchedules({ year })
    setSchedules(list)
  }, [year])
  ...
  const loadTable = useCallback(async () => {
    ...
    try {
      const data = await mallFetchScheduleTable(currentSchedule.id)
      setTableData(data)
    } finally {
  ...
  const loadDetails = useCallback(async () => {
    ...
    try {
      const list = await mallFetchDetailList({
        year, month,
  ...
  const handleEditSave = async () => {
    ...
    try {
      await mallEditDetail(currentSchedule.id, editingDetail.id, {
  ...
  const handleDeleteDetail = async (row: ScheduleDetailRow) => {
    ...
    try {
      await mallDeleteDetail(currentSchedule.id, row.id)
      message.success('已刪除')
      loadTable()
      loadDetails()
    } catch (e: any) {
  ...
  const handleAddDetail = async () => {
    ...
    try {
      ...
      await mallAddDetail(currentSchedule.id, {
  ...
  const handleDeleteSchedule = async () => {
    if (!currentSchedule) return
    try {
      await mallDeleteSchedule(currentSchedule.id)
