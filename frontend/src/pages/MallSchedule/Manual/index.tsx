/**
 * 商場班表 — 使用手冊（/mall/schedule/manual）
 *
 * 內容在 content.ts，這裡只負責版面：目錄、全文搜尋、區塊渲染。
 *
 * ⚠️ 刻意**不重用** `pages/Contract/Manual`、`pages/Schedule/Manual` 等其他模組的
 *    手冊頁面 —— 各模組彼此獨立，重用會讓其中一邊的改動波及另一邊。
 *    版面骨架與 Contract 版相同（複製而非引用），是刻意的重複。
 *
 * 權限：比照商場班表其他選單項目，掛 `mall_schedule_view`（見 MainLayout.tsx）——
...
const MallScheduleManualPage: React.FC = () => {
      ...
      <Row gutter={16}>
        ...
        <Col xs={24} md={18}>
          ...
          <Text type="secondary" style={{ fontSize: 12 }}>
            商場班表與飯店班表是完全獨立的兩套資料，
            這份手冊只涵蓋商場班表。另一邊的操作說明請看
            <Text code style={{ fontSize: 11 }}>/hotel/schedule/manual</Text>
            。
          </Text>
        </Col>
      </Row>
...
export default MallScheduleManualPage
