/**
 * 【商場班表】此檔為 pages/Schedule/Departments.tsx 的對稱複製版。
...
import { mallFetchDepartments, mallCreateDepartment, mallUpdateDepartment, mallDeleteDepartment } from '@/api/mallSchedule'
...
export default function DepartmentsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try { setData(await mallFetchDepartments()) }
  ...
  const handleSave = async () => {
    try {
      ...
      if (editing) {
        await mallUpdateDepartment(editing.id, values)
        message.success('部門已更新')
      } else {
        await mallCreateDepartment(values)
  ...
  return (
    <div style={{ padding: '24px' }}>
      <Card
        title={<Title level={4} style={{ margin: 0 }}>商場班表部門管理</Title>}
