...
import { mallImportExcel, mallFetchImportLogs } from '@/api/mallSchedule'
...
export default function ImportPage() {
  ...
  const loadLogs = useCallback(async () => {
    setLogsLoading(true)
    try { setLogs(await mallFetchImportLogs(10)) }
  ...
  const doImport = async (file: File, yr?: number, mo?: number) => {
    setStep(1)
    try {
      const res = await mallImportExcel(file, yr, mo)
      setResult(res)
  ...
  return (
    <div style={{ padding: '24px' }}>
      <Title level={4}>商場 Excel 班表匯入</Title>
