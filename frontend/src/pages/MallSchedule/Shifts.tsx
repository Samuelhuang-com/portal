/**
 * 【商場班表】此檔為 pages/Schedule/Shifts.tsx 的對稱複製版。
...
import { mallFetchShifts, mallCreateShift, mallUpdateShift, mallDeleteShift } from '@/api/mallSchedule'
...
export default function ShiftsPage() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editing) {
        await mallUpdateShift(editing.id, values)
        message.success('班別已更新')
      } else {
        await mallCreateShift(values)
  ...
  return (
    <div style={{ padding: '24px' }}>
      <Card
        title={<Title level={4} style={{ margin: 0 }}>商場班別管理</Title>}
        extra={
