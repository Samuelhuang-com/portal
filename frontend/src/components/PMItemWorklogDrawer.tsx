 ...
 * 保養項目明細 Drawer（共用元件）
 *
 * 2026-07-13 重新設計，嚴格遵循 CLAUDE.md §7／docs/WORK_JOURNAL_SPEC.md §9
 * 「明細 Drawer 強制規範（MANDATORY）」：
 ...
 *
 * 原本這個元件只顯示「維修記錄」子表（範圍太窄——使用者反映 Ragic 原始記錄裡類別/頻率/
 * 執行月份/排定人員/異常說明/附圖等欄位完全看不到）。現在改吃完整的 PMItem，元件自己負責
 * 打 worklogs + db-images 兩支 API，父元件只要管「開了誰」就好，不用各自維護三份 state。
 *
 * 資料來源：Ragic Sheet28（全棟週期保養日誌(同仁執行) - 子表:項目）單筆記錄；
 * 「維修記錄」巢狀子表格 → 本地 full_bldg_pm_item_worklog 表 → GET /items/{id}/worklogs
 * 「圖片上傳」欄位 → 本地 full_bldg_pm_batch_item.images_json → GET /items/{id}/db-images
 *
 * 已接上：Detail.tsx（批次明細頁）、FullBuildingMaintenance/index.tsx（Dashboard KPI 卡片
 * 明細 Drawer）、FullBldgDailyFormTab.tsx（每日巡檢表 TAB）。
 * 開發規範：本模組任何顯示「保養時間」欄位的項目表格，一律要能點擊該列開此 Drawer。
 */
...
import {
  fetchFullBldgPMItemWorklogs, fetchFullBldgPMItemImages,
  type PMWorklogItem, type PMImageItem,
} from '@/api/fullBuildingMaintenance'
import type { PMItem } from '@/types/periodicMaintenance'

const { Text } = Typography

const RAGIC_ITEM_BASE = 'https://ap12.ragic.com/soutlet001/periodic-maintenance/28'

const CATEGORY_COLOR: Record<string, string> = {
  水電: '#1890FF', 空調: '#13C2C2', 消防: '#FA541C',
  照明: '#FAAD14', 申報: '#722ED1', 整體: '#52C41A',
}
...
export interface PMItemWorklogDrawerProps {
  open:    boolean
  onClose: () => void
  item:    PMItem | null
}

export default function PMItemWorklogDrawer({ open, onClose, item }: PMItemWorklogDrawerProps) {
  ...
  useEffect(() => {
    ...
    fetchFullBldgPMItemWorklogs(item.ragic_id)
      .then(setWorklogs).catch(() => setWorklogs([])).finally(() => setWorklogsLoading(false))
    fetchFullBldgPMItemImages(item.ragic_id)
      ...
      title={item && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 16, color: '#1B3A5C', fontWeight: 600 }}>
            全棟例行維護：<span style={{ fontWeight: 400 }}>{item.task_name}</span>
          </span>
          {ragicUrl && (
            <a
              href={ragicUrl}
