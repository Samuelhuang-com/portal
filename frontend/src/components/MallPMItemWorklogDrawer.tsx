 ...
 * 商場週期保養項目明細 Drawer
 *
 * 2026-07-13 新增，比照 full_bldg_pm（全棟例行維護）PMItemWorklogDrawer.tsx 同日重新設計，
 * 嚴格遵循 CLAUDE.md §7／docs/WORK_JOURNAL_SPEC.md §9「明細 Drawer 強制規範（MANDATORY）」：
 ...
 *
 * 依專案既有慣例採「複製而非共用」：與 PMItemWorklogDrawer.tsx 邏輯完全相同，
 * 僅 Ragic 連結基準與標題前綴改為商場週期保養（Sheet24），並改打
 * mallPeriodicMaintenance.ts 的 worklogs/db-images API。
 *
 * 資料來源：Ragic Sheet24（商場週期保養日誌(同仁執行) - 子表:項目）單筆記錄；
 * 「維修記錄」巢狀子表格 → 本地 mall_pm_item_worklog 表 → GET /items/{id}/worklogs
 * 「圖片上傳」欄位 → 本地 mall_pm_batch_item.images_json → GET /items/{id}/db-images
 *
...
import {
  fetchMallPMItemWorklogs, fetchMallPMItemImages,
...
const RAGIC_ITEM_BASE = 'https://ap12.ragic.com/soutlet001/periodic-maintenance/24'
...
export interface MallPMItemWorklogDrawerProps {
  open:    boolean
  onClose: () => void
  item:    PMItem | null
}

export default function MallPMItemWorklogDrawer({ open, onClose, item }: MallPMItemWorklogDrawerProps) {
  ...
  useEffect(() => {
    ...
    fetchMallPMItemWorklogs(item.ragic_id)
      .then(setWorklogs).catch(() => setWorklogs([])).finally(() => setWorklogsLoading(false))
    fetchMallPMItemImages(item.ragic_id)
      ...
      title={item && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 16, color: '#1B3A5C', fontWeight: 600 }}>
            商場週期保養：<span style={{ fontWeight: 400 }}>{item.task_name}</span>
          </span>
          {ragicUrl && (
            <a
              href={ragicUrl}
