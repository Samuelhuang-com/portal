/**
 * 台中核准請購單月報表 API（2026-09-16 完整複製自 purchaseReport.ts）
 * 所有 taichung-purchase-report 相關 API 呼叫的封裝函數
 */
import apiClient from '@/api/client'

const BASE = '/taichung-purchase-report'
...
export interface PurchaseReportItem {
  ...
  item_remark: string
  taxable_amount: number | null  // 應稅金額（台中）
  // 台中額外欄位
  subject: string | null
  reason_31: string | null
  final_vendor_no: string | null
  final_vendor_name: string | null
  final_payee: string | null
  final_bank_name: string | null
  final_bank_account: string | null
  final_bank_code: string | null
...
/** 會科清單（下拉選項） */
export const getPurchaseAccountCategories = (company = '台中') =>
...
export const getPurchaseAvailableMonths = (company = '台中') =>
...
export interface PurchaseOrder {
  ...
  remark: string | null
  // 台中額外欄位
  subject: string | null             // 主旨
  reason_31: string | null           // 31事由
  final_vendor_no: string | null     // 24最終擬定廠商編號
  final_vendor_name: string | null   // 25擬定廠商名稱
  final_payee: string | null         // 26擬定受款者
  final_bank_name: string | null     // 27擬定受款銀行
  final_bank_account: string | null  // 28擬定匯款帳號
  final_bank_code: string | null     // 29擬定銀行代號
  currency: string | null            // 幣別
  request_date: string | null
  approved_date: string | null
