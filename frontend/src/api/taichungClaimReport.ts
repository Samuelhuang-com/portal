/**
 * 台中核准請款單月報表 API（2026-09-16 完整複製自 claimReport.ts）
 * 所有 taichung-claim-report 相關 API 呼叫的封裝函數
 */
import apiClient from '@/api/client'

const BASE = '/taichung-claim-report'
...
export interface ClaimOrder {
  ...
  // 台中額外欄位
  total_amount: number | null     // 總金額
  advance_offset: number | null   // 沖預支
  currency: string | null         // 幣別
  payment_bank: string | null     // 付款銀行
  status: string
...
/** 有資料的年月清單 */
export const getClaimAvailableMonths = (company = '台中') =>
...
/** 會科清單（下拉） */
export const getClaimAccountSubjects = (company = '台中') =>
