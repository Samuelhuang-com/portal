/**
 * 大直工務部 API 封裝
 * 所有對 /api/v1/dazhi-repair/* 的請求統一在此處理
 */
...
export async function fetchDetail(params: DetailQueryParams): Promise<DetailResult> {
  ...
  if (params.record_status !== undefined && params.record_status.length > 0) p.record_status = params.record_status.join(',')
...
export function buildExportUrl(params: {
  ...
  floor?: string; status?: string[]; record_status?: string[]; keyword?: string;
}): string {
  ...
  if (params.record_status !== undefined && params.record_status.length > 0) query.set('record_status', params.record_status.join(','))
