...
import type {
  CadenceResult, DashboardResult, FetchLogRow, FetchRunResult, HotelSearchResult,
  ImportResult, MatrixResult, QuotaGrantRow, RateDetail, SubscriberDetail,
  TrendResult,
} from '@/types/compset'
 ...
 * 搜尋競爭組候選。
 *
 * ⚠️⚠️ **會扣配額**（每頁 1 次 SerpApi 查詢 ＝ 真的錢）。
 *    呼叫端一定要先讓使用者看到成本，不要在開啟畫面時自動呼叫。
 * ⚠️ 402 ＝ 配額不足。那不是錯誤是狀態，訊息要原樣顯示給使用者。
 */
export async function searchHotels(params: {
  q?: string; pages?: number; checkInDate?: string; subscriberId?: number
}): Promise<HotelSearchResult> {
  const { data } = await apiClient.post(`${BASE}/hotels/search`, {
    q: params.q ?? '', pages: params.pages ?? 1,
    check_in_date: params.checkInDate ?? '',
  }, { params: toParams({ subscriber_id: params.subscriberId }) })
  return data
}
