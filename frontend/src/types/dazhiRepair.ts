...
// ── 4.4 客房報修表 ────────────────────────────────────────────────────────────
// 每個格子的條目現在是完整 RepairCase（後端 compute_room_repair_table 回傳 case.to_dict()）
export type RoomCategoryEntry = RepairCase

export interface RoomRepairRow {
  room_no:    string
  floor:      string
  categories: Record<string, RepairCase[]>
}
...
// ── 金額統計 ──────────────────────────────────────────────────────────────────
// 大直工務部目前有 3 項費用（無「扣款專櫃」，Ragic 補上後可加入）
export type FeeKey = 'outsource_fee' | 'maintenance_fee' | 'deduction_fee'
