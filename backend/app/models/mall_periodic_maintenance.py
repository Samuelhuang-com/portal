"""
商場週期保養表 SQLAlchemy ORM Models

資料表：
  mall_pm_batch       — 保養批次主表（對應 Ragic Sheet 18）
  mall_pm_batch_item  — 保養項目明細（對應 Ragic Sheet 18 sub-table）
"""
import json
...
class MallPeriodicMaintenanceBatch(Base):
    """商場保養批次主表（每月/每期一筆）"""
    __tablename__ = "mall_pm_batch"

    # ── 主鍵 ─────────────────────────────────────────────────────────────────
    ragic_id = Column(String(50), primary_key=True, comment="Ragic Sheet 18 記錄 ID")

    # ── 業務欄位 ──────────────────────────────────────────────────────────────
    journal_no    = Column(String(50),  nullable=False, default="", comment="保養日誌編號，如 商週保202604-001")
...
class MallPeriodicMaintenanceItem(Base):
    """商場保養項目明細（每個保養任務一筆）"""
    __tablename__ = "mall_pm_batch_item"

    # ── 主鍵 ─────────────────────────────────────────────────────────────────
    ragic_id = Column(String(50), primary_key=True, comment="Ragic 記錄 ID，格式 {batch_id}_{row_key}")

    # ── 外鍵關聯 ──────────────────────────────────────────────────────────────
    batch_ragic_id = Column(String(50), nullable=False, default="", comment="關聯 mall_pm_batch.ragic_id")
    ...
    # ── 附圖（2026-07-13 新增，來源 Ragic Sheet24「圖片上傳」欄位）───────────
    # 格式：JSON list，元素為 {"url": "...", "filename": "..."}（見 ragic_data_service.parse_images）
    images_json = Column(Text, nullable=True, default=None, comment="附圖 JSON，來源 Ragic Sheet24「圖片上傳」欄位")
    ...
    def __repr__(self):
        return f"<MallPMItem ragic_id={self.ragic_id} seq_no={self.seq_no} task_name={self.task_name[:20]}>"


class MallPMItemWorklog(Base):
    """
    商場週期保養項目執行記錄明細（每個保養項目底下可有多筆維修記錄）

    資料來源：Ragic Sheet 24（商場週期保養日誌(同仁執行) - 子表:項目）
              每筆記錄的巢狀子表格「維修記錄」（Ragic 內部 key 為 _subtable_<動態數字>，
              需以 fetch_one() 逐筆項目取得，無法從 fetch_all() 列表模式取得）

    2026-07-13 新增：比照全棟例行維護（full_bldg_pm）Sheet28 改版同一模式，將 mall_pm
    的項目明細主要來源改為 Sheet24，並同步逐筆維修記錄明細（時間起訖、保養人員、
    維修記錄文字），取代原本只同步彙總欄位「維修工時」到 repair_hours 的做法。
    """
    __tablename__ = "mall_pm_item_worklog"
    ...
    # ── 外鍵關聯 ──────────────────────────────────────────────────────────────
    item_ragic_id = Column(String(50), nullable=False, default="", comment="關聯 mall_pm_batch_item.ragic_id")
    ...
    def __repr__(self):
        return f"<MallPMItemWorklog ragic_id={self.ragic_id} item_ragic_id={self.item_ragic_id}>"
