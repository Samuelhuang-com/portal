...
資料表：pm_schedule
  ...
  - 由「產生本月排程」功能依 pm_batch_item 主檔的頻率規則自動建立
...
class PMSchedule(Base):
    """週期保養排程明細（Portal 自有，不影響 Ragic 同步）"""
    ...
    item_ragic_id = Column(String(50),  nullable=False, comment="來源主檔 pm_batch_item.ragic_id")
    ...
    __table_args__ = (
        UniqueConstraint("year_month", "item_ragic_id", name="uq_pm_schedule_month_item"),
        Index("ix_pm_schedule_year_month", "year_month"),
        Index("ix_pm_schedule_item",       "item_ragic_id"),
        Index("ix_pm_schedule_completed",  "is_completed"),
        Index("ix_pm_schedule_abnormal",   "abnormal_flag"),
    )

    def __repr__(self):
        return (
            f"<PMSchedule id={self.id} year_month={self.year_month} "
