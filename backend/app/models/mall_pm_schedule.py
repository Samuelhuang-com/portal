"""
商場週期保養排程明細 ORM Model

資料表：mall_pm_schedule
  - Portal 自有排程記錄（與 Ragic 同步無關）
  - 由「產生本月排程」功能依 mall_pm_batch_item 主檔的頻率規則自動建立
...
class MallPMSchedule(Base):
    """商場週期保養排程明細（Portal 自有，不影響 Ragic 同步）"""
    __tablename__ = "mall_pm_schedule"
    ...
    item_ragic_id = Column(String(50),  nullable=False, comment="來源主檔 mall_pm_batch_item.ragic_id")
    ...
    __table_args__ = (
        UniqueConstraint("year_month", "item_ragic_id", name="uq_mall_pm_schedule_month_item"),
        Index("ix_mall_pm_schedule_year_month", "year_month"),
        Index("ix_mall_pm_schedule_item",       "item_ragic_id"),
        Index("ix_mall_pm_schedule_completed",  "is_completed"),
        Index("ix_mall_pm_schedule_abnormal",   "abnormal_flag"),
    )

    def __repr__(self):
        return (
            f"<MallPMSchedule id={self.id} year_month={self.year_month} "
