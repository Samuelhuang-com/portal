"""
整棟工務每日巡檢 - B2F  ORM 模型【寬表格 Pivot 架構】
...
class B2FInspectionBatch(Base):
    """一次完整巡檢場次，對應 Ragic Sheet 3 的一個 Row。"""
    __tablename__ = "b2f_inspection_batch"

    ragic_id = Column(String(50), primary_key=True, comment="Ragic Sheet 3 記錄 ID")
...
class B2FInspectionItem(Base):
    """巡檢設備項目（一筆 = 一個設備欄位值）。"""
    __tablename__ = "b2f_inspection_item"
