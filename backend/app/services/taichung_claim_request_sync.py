"""
台中核准請款單同步服務：Ragic → PostgreSQL（2026-09-16 完整複製自樂群 claim_request_sync.py）

架構與樂群相同：Master + Detail 雙層同步
  Step 1（每 15 分鐘）：清單 API × 10 個表單 → taichung_claim_requests（主單）
  Step 2（每 45 分鐘）：subtable 解析 or 內頁 API → taichung_claim_request_items（品項）

【與樂群的差異（2026-09-16 實測 Ragic ＋ 使用者裁示）】
  1. 部門顯示名稱一律取表單歸屬（dept_config["display_name"]）。台中請款單沒有「部門」
     欄位，只有「申請部門」。
  2. 核准日期：台中請款單沒有「日期N」，簽核時間在「簽署日期～簽署日期6」→ 取最大值
     （使用者裁示）；全空才退回申請日期。
  3. 單號欄位是「資請編號」（值：台公請YYYYMMDDNNN），各部門會重複。
  4. 品項金額欄位是「擬定廠商金額(未稅)」；「擬定廠商金額(未稅)2」實測 565 列全為 0，不採用。
  5. 事由欄：優先「說明」，沒有才用「主旨」（使用者裁示）；台中請款的內容多半寫在主旨。
  6. 額外欄位：總金額、沖預支、幣別、付款銀行
"""
import json
import logging
import re
...
LIST_FIELD_CANDIDATES: dict[str, list[str]] = {
    ...
    "purpose_description":["說明", "事由", "事由/說明", "摘要", "主旨"],   # 台中：優先說明，沒有才主旨
    "subtotal":           ["小計", "未稅小計"],
    "tax":                ["營業稅", "稅額", "稅金", "稅"],
    "total":              ["總計", "含稅總計", "合計"],
...
DETAIL_FIELD_CANDIDATES: dict[str, list[str]] = {
    ...
    "purpose_description":["說明", "事由", "事由/說明", "主旨"],
    "voucher_no":         ["傳票號碼"],
}
...
ITEM_FIELD_CANDIDATES: dict[str, list[str]] = {
    ...
    "unit":                   ["單位"],
    "item_note":              ["品項備註", "備註"],
    "proposed_vendor_amount": ["擬定廠商金額(未稅)", "擬定廠商金額", "廠商金額", "金額", "請款金額"],
    "invoice_no":             ["發票號碼", "發票"],
...
# ── 請款單號 regex pattern ────────────────────────────────────────────────────
# 格式：台 + 任意中文/英文 + 請 + 8位日期 + 3位流水號
# 例：台公請20260708004
_REQUEST_NO_RE = re.compile(r"^台.+請\d{8,}")
...
def _dept_display(dept_config: dict) -> str:
    """台中：部門顯示名稱一律取表單歸屬（使用者 2026-09-16 裁示）。"""
    return dept_config.get("display_name", "")
...
def _parse_list_record(
    ...
    # 1. 台中：Ragic 工作流「簽署日期 / 簽署日期2 … 簽署日期6」取最大值（使用者 2026-09-16 裁示）
    # 2. 明確命名的核准/付款日期欄位
    # 3. 最終備援：申請日期（比最後更新更接近實際審核時間）
    approved_dt = None
    if status == "F":
        date_vals = []
        # 第一層：簽署日期 / 簽署日期2 … 工作流簽核時間戳（台中請款單沒有「日期N」）
        for k, v in data.items():
            if re.match(r"^簽署日期\d*$", k) and v:
                d = _to_date(v)
                if d:
                    date_vals.append(d)
        if date_vals:
    ...
    dept_display = _dept_display(dept_config)

    return {
        "company":                "台中",
        "department_raw":         (raw_dept or dept_config.get("ragic_dept", ""))[:20],
        ...
        # ── 台中額外欄位 ──
        "total_amount":           _to_int(_pick(data, LIST_FIELD_CANDIDATES["total_amount"])),
        "advance_offset":         _to_int(_pick(data, LIST_FIELD_CANDIDATES["advance_offset"])),
        "currency":               str(_pick(data, LIST_FIELD_CANDIDATES["currency"], "")).strip()[:20] or None,
        "payment_bank":           str(_pick(data, LIST_FIELD_CANDIDATES["payment_bank"], "")).strip()[:200] or None,
        "status":                 status,
...
async def _sync_list_for_dept(
    ...
    try:
        ...
        logger.error("[TaichungClaimSync][List] %s 清單 API 失敗：%s", display, exc)
            ...
            with db.begin_nested():  # SAVEPOINT：單筆失敗只 rollback 該筆，不影響其他記錄
                ...
                if not fields.get("detail_synced", False):
                    ...
                    if subtable_rows:
                        ...
                        logger.debug("[TaichungClaimSync][List] record %s 從清單解析到 %d 筆品項",
                                     record_id, len(subtable_rows))

            upserted += 1
        except Exception as exc:
            errors.append(f"{display}[{record_id}]: {exc}")
            logger.warning("[TaichungClaimSync][List] %s[%s] 解析失敗: %s", display, record_id, exc)

    try:
        db.commit()
    except Exception as exc:
        db.rollback()
        logger.error("[TaichungClaimSync][List] %s commit 失敗: %s", display, exc)
...
async def sync_list_only() -> dict:
    ...
    try:
        ...
        valid_paths = {d["list_path"] for d in get_sheet_configs("taichung_claim")}
        orphan_ids = [
            row.id
        ...
        if orphan_ids:
            ...
            db.commit()
            logger.info("[TaichungClaimSync] 清除孤兒記錄 %d 筆（路徑已失效）", len(orphan_ids))

        for dept in get_sheet_configs("taichung_claim"):
        ...
        db.close()
    logger.info("[TaichungClaimSync] 清單同步完成：%s", total)
    return total
...
async def sync_from_ragic(full_resync: bool = False) -> dict:
    ...
    _dept_sheets = get_sheet_configs("taichung_claim")
    ...
    try:
        ...
        for i, order in enumerate(pending):
            ...
            if not detail_path:
                logger.warning("[TaichungClaimSync][Detail] 找不到 detail_path for %s", order.ragic_sheet_path)
                continue
            ...
            try:
                ...
                logger.warning("[TaichungClaimSync][Detail] record %s fetch/parse 失敗：%s",
        ...
        logger.error("[TaichungClaimSync] 完整同步失敗: %s", exc, exc_info=True)
    finally:
        db.close()

    logger.info("[TaichungClaimSync] 完整同步完成：%s", total)
    return total
