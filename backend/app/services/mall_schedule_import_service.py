"""
商場班表 Excel 匯入 Service
支援民國年 Sheet 格式（如：115年5月）
自動辨識日期列、姓名欄，處理未知班別與新增人員。

⚠️ 本檔為 app/services/schedule_import_service.py（飯店班表）的對稱複製版，
   解析邏輯必須與飯店版保持一致，修改任一邊時請同步檢視另一邊。
   新建人員一律帶 venue_flag「商」，不與飯店人員做任何比對。
"""
import re
import uuid
import logging
...
from app.models.mall_schedule import (
    MallSchedule, MallScheduleDetail, MallScheduleImportLog,
    MallStaffMember, MallShiftType, VENUE_FLAG,
)
...
def import_excel(
    ...
    # ── 9. 逐列讀取班表資料 ──────────────────────────────────
    details: list[MallScheduleDetail] = []
    ...
    for row_vals in all_rows[data_start_row:]:
        ...
        if not staff:
            ...
            logger.info("mall_schedule_import: 新增人員 %s", raw_name_str)
    ...
    # ── 11. 寫入匯入紀錄 ─────────────────────────────────────
    import_log = MallScheduleImportLog(
        import_batch_id=batch_id,
        file_name=file_name,
    ...
    logger.info(
        "mall_schedule_import: %d/%d 完成，明細 %d 筆，警告 %d，未知班別 %s",
