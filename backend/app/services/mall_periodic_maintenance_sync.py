        ...
        for item_ragic_id, raw in raw_data.items():
            ...
            try:
                ...
                try:
                    ...
                    raw_images_val = full_record.get(CK24L_IMAGES)
                    if not raw_images_val:
                        # 2026-07-14 新增：CK24L_IMAGES（中文欄位 label）當時是「新增」而非「實測驗證」
                        ...
                        fallback_key = next(
                            (k for k in full_record.keys()
                             if isinstance(k, str) and k != CK24L_IMAGES
                        ...
                        if fallback_key:
                            ...
                            if fallback_key not in _images_fallback_keys_seen:
                                ...
                                logger.warning(
                                    f"[MallPMSync][Sheet24Items] CK24L_IMAGES=\"{CK24L_IMAGES}\" 抓不到資料，"
