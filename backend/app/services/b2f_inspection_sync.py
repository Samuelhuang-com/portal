"""
整棟工務每日巡檢 - B2F 同步服務【寬表格 Pivot 架構 + 動態欄位偵測】

資料來源：
  https://ap12.ragic.com/soutlet001/full-building-inspection/3

【結構說明】
  Ragic Sheet 3 每一 Row = 一次完整巡檢場次（寬表格格式）
...
# ── Ragic 連線設定 ────────────────────────────────────────────────────────────
B2F_SERVER_URL = getattr(settings, "RAGIC_B2F_SERVER_URL",  "ap12.ragic.com")
B2F_ACCOUNT    = getattr(settings, "RAGIC_B2F_ACCOUNT",     "soutlet001")
B2F_SHEET_PATH = getattr(settings, "RAGIC_B2F_SHEET_PATH",  "full-building-inspection/3")
...
@register("b2f_inspection")
async def sync_from_ragic() -> dict:
    """
    從 Ragic Sheet 3 同步：
      每個 Row → 1 筆 B2FInspectionBatch + pivot 成 N 筆 B2FInspectionItem。
    """
    ...
    logger.info("[B2FSync] 開始同步（寬表格 Pivot + 動態欄位偵測）...")

    try:
        raw_data = await adapter.fetch_all()
    except Exception as exc:
        logger.error(f"[B2FSync] 拉取失敗：{exc}")
    ...
    if fetched > 0:
        ...
        if measure_fields:
            logger.info(
                f"[B2FSync] 量測/程度型欄位 {len(measure_fields)} 個："
    ...
    try:
        for ragic_id, raw in raw_data.items():
            ...
            try:
                ...
                existing_batch = db.get(B2FInspectionBatch, batch_id)
                if existing_batch:
                ...
                logger.warning(f"[B2FSync] 記錄 {ragic_id} 失敗：{exc}")

        db.commit()
        logger.info(
            f"[B2FSync] 完成：fetched={fetched}, batches_upserted={upserted}, "
            f"items={item_rows}, errors={len(errors)}"
        )
    except Exception as exc:
        db.rollback()
        errors.append(f"DB commit error: {exc}")
        logger.error(f"[B2FSync] DB 寫入失敗：{exc}")
    finally:
        db.close()
