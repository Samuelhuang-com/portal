...
async def sync_from_ragic() -> dict:
    ...
    try:
        ...
        existing_map: dict = {
            row.ragic_id: row
            for row in db.query(DazhiRepairCase).filter(
                DazhiRepairCase.ragic_id.in_(all_ragic_ids)
            ).all()
        }
        ...
        records = 0
        try:
            db.query(DazhiRepairRecord).filter(
                DazhiRepairRecord.parent_ragic_id.in_(all_ragic_ids)
            ).delete(synchronize_session=False)
            for case in cases:
                for rec in extract_detail_records(getattr(case, "_raw", None) or {}):
                    db.add(DazhiRepairRecord(
