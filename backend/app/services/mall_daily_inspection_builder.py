...
build_mall_daily_inspection_table(year, month, db, inspection_date=None)
  → 將 MALL_DAILY_INSPECTION_TEMPLATE 的標準模板與 DB 實際巡檢資料比對，
    回傳完整的每日巡檢表列（含巡檢人員、結果、異常說明等）。
...
from app.services.mall_daily_inspection_template import (
    MALL_DAILY_INSPECTION_TEMPLATE,
    MallDailyInspectionTemplateRow,
)
...
def _load_sheet_data(
...
) -> list[tuple[MallFIBatch, list[MallFIItem]]]:
    """
    ...
    if inspection_date:
        q = q.filter(MallFIBatch.inspection_date == inspection_date)
    else:
        q = q.filter(MallFIBatch.inspection_date.like(f"{year_month_prefix}%"))
    batches = q.order_by(MallFIBatch.inspection_date.asc()).all()
...
def build_mall_daily_inspection_table(
    year: int,
    month: int,
    db: Session,
    inspection_date: str | None = None,
    ...
    """
    回傳每日巡檢表完整列（list of dict），依 MALL_DAILY_INSPECTION_TEMPLATE 順序。
    ...
    sheet_data: dict[str, list[tuple[MallFIBatch, list[MallFIItem]]]] = {}
    unique_tabs = set(row["source_tab"] for row in MALL_DAILY_INSPECTION_TEMPLATE)
    ...
    for tmpl_row in MALL_DAILY_INSPECTION_TEMPLATE:
