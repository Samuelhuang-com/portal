"""
台中核准請購單同步服務：Ragic → PostgreSQL（2026-09-16 完整複製自樂群 purchase_request_sync.py）

架構與樂群相同：Master + Detail 雙層同步
  Step 1：清單 API × 10 個表單 → taichung_purchase_requests（主單）
          清單 API 已含 _subtable_* 品項，直接寫 taichung_purchase_request_items
  Step 2：Detail API（僅補 detail_synced=False 的記錄）

【與樂群的差異（2026-09-16 實測 Ragic ＋ 使用者裁示）】
  1. 部門顯示名稱一律取表單歸屬（dept_config["display_name"]），不看 Ragic「部門」欄位值
     —— 公用表單的單 Ragic 部門值是「資訊」，但不能算進資訊部。
  2. 金額口徑：台中表單沒有「全案小計／全案總計」。
       小計／稅／總計   ＝ 廠商(一) 的小計／稅／總計（比價用，不是最終金額）
       小計4／營業稅／總計4 ＝ 擬定廠商的金額（＝品項 金額4 加總）
     資訊部表單把「小計4」「金額4」命名為「小計(未稅)」「金額(未稅)」。
     實測 175 張中有 7 張 小計≠小計4（三家比價、擬定非廠商一），取錯就會跟品項對不起來。
  3. 額外欄位：主旨、31事由、24/25 擬定廠商編號與名稱、26~29 擬定受款資訊、幣別；品項應稅金額
  4. 說明欄：優先「說明」，沒有才用「主旨」（使用者裁示，與樂群順序一致）
"""
import asyncio
import json
import logging
...
LIST_FIELD_CANDIDATES: dict[str, list[str]] = {
    ...
    "amount":           ["小計4", "小計(未稅)", "全案小計", "小計", "金額"],   # ⚠️ 台中：小計4＝擬定、小計＝廠商一
    "status":           ["簽核狀態", "狀態"],
    ...
    "amount_tax":       ["營業稅", "稅額", "稅金", "稅"],   # ⚠️ 台中：營業稅＝擬定、稅＝廠商一
    "amount_total":     ["總計4", "全案總計", "含稅總額", "全案合計"],   # ⚠️ 台中：總計4＝擬定、總計＝廠商一
    "remark":           ["備註", "補充"],
    # ── 台中額外欄位（2026-09-16）──
    "subject":            ["主旨"],
    "reason_31":          ["31事由"],
    "final_vendor_no":    ["24最終擬定廠商編號"],
    "final_vendor_name":  ["25擬定廠商名稱"],
    "final_payee":        ["26擬定受款者"],
    "final_bank_name":    ["27擬定受款銀行"],
    "final_bank_account": ["28擬定匯款帳號"],
    "final_bank_code":    ["29擬定銀行代號"],
    "currency":           ["幣別"],
}
...
DETAIL_FIELD_CANDIDATES: dict[str, list[str]] = {
    ...
    "amount_total":     ["總計4", "全案總計", "含稅總額"],   # ⚠️ 不可含「總計」（台中＝廠商一總計）
...
ITEM_FIELD_CANDIDATES: dict[str, list[str]] = {
    ...
    "qty":              ["數量"],
    "unit":             ["單位"],
    "item_remark":      ["品項備註", "品項備註/上期單價", "品項備註(含上期單價)", "備註"],
    ...
    "selected_amount":  ["擬定金額", "金額4", "金額(未稅)"],   # 資訊部表單叫「金額(未稅)」
    "taxable_amount":   ["應稅金額"],
    "is_confirmed":     ["勾選"],
}
...
def _person_name(val: Any, limit: int = 50) -> str | None:
    ...
    if len(s) > limit:
        ...
        logger.warning("[TaichungPurchaseSync] applicant 長度 %d 超過 %d，已捨棄：%s…",
                       len(s), limit, s[:40])
        return None
    return s
...
def _dept_display(dept_config: dict) -> str:
    """台中：部門顯示名稱一律取表單歸屬（使用者 2026-09-16 裁示），不看 Ragic 部門值。"""
    return dept_config["display_name"]
...
def _parse_list_record(
    ...
    return {
        "company":            "台中",
        "department_raw":     (raw_dept or dept_config["ragic_dept"])[:20],
        "department_display": _dept_display(dept_config),
        ...
        # ── 台中額外欄位 ──
        "subject":            str(_pick(data, LIST_FIELD_CANDIDATES["subject"], "")).strip() or None,
        "reason_31":          str(_pick(data, LIST_FIELD_CANDIDATES["reason_31"], "")).strip() or None,
        "final_vendor_no":    str(_pick(data, LIST_FIELD_CANDIDATES["final_vendor_no"], "")).strip()[:30] or None,
        "final_vendor_name":  str(_pick(data, LIST_FIELD_CANDIDATES["final_vendor_name"], "")).strip()[:200] or None,
        "final_payee":        str(_pick(data, LIST_FIELD_CANDIDATES["final_payee"], "")).strip()[:200] or None,
        "final_bank_name":    str(_pick(data, LIST_FIELD_CANDIDATES["final_bank_name"], "")).strip()[:200] or None,
        "final_bank_account": str(_pick(data, LIST_FIELD_CANDIDATES["final_bank_account"], "")).strip()[:50] or None,
        "final_bank_code":    str(_pick(data, LIST_FIELD_CANDIDATES["final_bank_code"], "")).strip()[:30] or None,
        "currency":           str(_pick(data, LIST_FIELD_CANDIDATES["currency"], "")).strip()[:20] or None,
...
async def _sync_list_for_dept(dept_config: dict, db) -> dict:
    ...
    try:
        ...
        logger.error("[TaichungPurchaseSync][List] %s 清單 API 失敗：%s", dept_config["display_name"], exc)
    ...
    for record_id, data in records.items():
        ...
        try:
            with db.begin_nested():  # SAVEPOINT：單筆失敗只 rollback 該筆，不影響其他記錄
                ...
                if not fields.get("detail_synced", False):
                    ...
                    if item_rows:
                        ...
                        logger.debug(
                            "[TaichungPurchaseSync][List] record %s 從清單資料解析到 %d 筆品項",
            ...
            logger.warning("[TaichungPurchaseSync][List] record %s 解析失敗：%s", record_id, exc)
    ...
    try:
        ...
        db.rollback()
        logger.error("[TaichungPurchaseSync][List] commit 失敗：%s", exc)
...
def _parse_item_row(row: dict, order_id: int, idx: int) -> dict:
    ...
    return {
        ...
        "taxable_amount":     _to_int(_pick(row, ITEM_FIELD_CANDIDATES["taxable_amount"])),
        "sync_at":            twnow(),
    }
...
async def _sync_detail_batch(orders: list[ApprovedPurchaseRequest], db) -> dict:
    ...
    sheet_to_detail: dict[str, str] = {
        d["list_path"]: d["detail_path"] for d in get_sheet_configs("taichung_purchase")
    }

    for order in orders:
        detail_path = sheet_to_detail.get(order.ragic_sheet_path)
        if not detail_path:
            logger.warning("[TaichungPurchaseSync][Detail] 找不到 detail_path for %s", order.ragic_sheet_path)
        ...
        try:
            ...
            logger.warning("[TaichungPurchaseSync][Detail] record %s fetch 失敗：%s",
        ...
        try:
            ...
            logger.warning("[TaichungPurchaseSync][Detail] record %s 解析失敗：%s",
                           order.ragic_record_id, exc)
    ...
    try:
        ...
        db.rollback()
        logger.error("[TaichungPurchaseSync][Detail] commit 失敗：%s", exc)
        errors.append(f"commit: {exc}")
...
async def sync_list_only() -> dict:
    ...
    for dept in get_sheet_configs("taichung_purchase"):
        db = SessionLocal()
        try:
    ...
    logger.info("[TaichungPurchaseSync] 清單同步完成：%s", list_result)
    return list_result
...
async def sync_from_ragic(full_resync: bool = False) -> dict:
    ...
    try:
        # ════════════════════════════════════════════════════
        # Step 1：清單 API（10 個表單）— 每部門獨立 session
        # ════════════════════════════════════════════════════
        _dept_sheets = get_sheet_configs("taichung_purchase")
        logger.info(f"[TaichungPurchaseSync] Step 1 開始：清單同步（{len(_dept_sheets)} 部門）")
        for dept in _dept_sheets:
            ...
            list_result["errors"].extend(result["errors"])
            logger.info(
                "[TaichungPurchaseSync][List] %s：fetched=%d upserted=%d errors=%d",
                dept["display_name"],
                result["fetched"],
                result["upserted"],
                len(result["errors"]),
        ...
        # ════════════════════════════════════════════════════
        logger.info("[TaichungPurchaseSync] Step 2 開始：Detail 同步（full_resync=%s）", full_resync)
        ...
        logger.info("[TaichungPurchaseSync][Detail] 待同步筆數：%d", len(pending_ids))
        ...
        for i in range(0, len(pending_ids), BATCH_SIZE):
            ...
            logger.info(
                "[TaichungPurchaseSync][Detail] 批次 %d/%d（%d 筆）",
        ...
        logger.error("[TaichungPurchaseSync] 同步失敗：%s", exc)
        list_result["errors"].append(str(exc))

    logger.info(
        "[TaichungPurchaseSync] 完成 list=(%d/%d) detail=(%d/%d) errors=%d",
