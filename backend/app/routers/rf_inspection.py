...
from app.services.rf_inspection_sync import (
    sync_from_ragic, RF_SERVER_URL, RF_ACCOUNT, RF_SHEET_PATH,
)
...
async def verify_count(db: Session = Depends(get_db)):
    portal_count, last_synced_at = await read_portal_count_and_last_sync(
        db, RFInspectionBatch, "RF巡檢"
    )
    try:
        ragic_count = await fetch_ragic_count(RF_SHEET_PATH, RF_SERVER_URL, RF_ACCOUNT)
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"Ragic 連線失敗：{exc}")
    return build_verify_count_response("RF巡檢", portal_count, ragic_count, last_synced_at)
...
async def verify_diff(db: Session = Depends(get_db)):
    try:
        ragic_url_map = await fetch_ragic_url_map_single(RF_SHEET_PATH, RF_SERVER_URL, RF_ACCOUNT)
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"Ragic 連線失敗：{exc}")
    portal_ids = await read_portal_ragic_ids(db, RFInspectionBatch)
...
def get_batch_db_images(batch_ragic_id: str, db: Session = Depends(get_db)):
    ...
    items = (
        db.query(RFInspectionItem)
        .filter(RFInspectionItem.batch_ragic_id == batch_ragic_id)
        .order_by(RFInspectionItem.seq_no)
        .all()
    )
    ...
    for it in items:
        ...
        for token in split_attachment_value(it.result_raw):
            images.extend(parse_images(
                token, server=RF_SERVER_URL, account=RF_ACCOUNT,
            ))
    return images
