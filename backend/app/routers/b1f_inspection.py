"""
整棟工務每日巡檢 - B1F  API Router【寬表格 Pivot 架構】
Prefix: /api/v1/mall/b1f-inspection
...
from app.schemas.b1f_inspection import (
...
def _batch_to_out(b: B1FInspectionBatch, item_count: int = 0) -> B1FInspectionBatchOut:
    return B1FInspectionBatchOut(
        ragic_id        = b.ragic_id,
...
def _item_to_out(it: B1FInspectionItem) -> B1FInspectionItemOut:
    return B1FInspectionItemOut(
...
def _abnormal_count_for_batches(db, batches) -> int:
    ...
    total = 0
    for b in batches:
        items = db.query(B1FInspectionItem).filter(
            B1FInspectionItem.batch_ragic_id == b.ragic_id
        ).all()
        total += sum(
...
def _calc_kpi(items: list[B1FInspectionItem]) -> B1FInspectionBatchKPI:
...
@router.post("/sync", summary="從 Ragic 同步 B1F 巡檢資料（背景執行）", dependencies=[Depends(require_roles("system_admin", "module_manager"))])
async def sync_b1f_inspection(background_tasks: BackgroundTasks):
...
def get_batch_db_images(batch_ragic_id: str, db: Session = Depends(get_db)):
    ...
    items = (
        db.query(B1FInspectionItem)
        .filter(B1FInspectionItem.batch_ragic_id == batch_ragic_id)
        .order_by(B1FInspectionItem.seq_no)
        .all()
    )
    ...
    for it in items:
        ...
        for token in split_attachment_value(it.result_raw):
            images.extend(parse_images(
                token, server=B1F_SERVER_URL, account=B1F_ACCOUNT,
            ))
    return images
...
def list_batches(
    ...
    q = db.query(B1FInspectionBatch)
    if year_month:
        q = q.filter(B1FInspectionBatch.inspection_date.like(f"{year_month}%"))
    if start_date:
        q = q.filter(B1FInspectionBatch.inspection_date >= start_date)
    if end_date:
        q = q.filter(B1FInspectionBatch.inspection_date <= end_date)

    batches = q.order_by(B1FInspectionBatch.inspection_date.desc()).all()
    result = []
    for b in batches:
        items = db.query(B1FInspectionItem).filter(
            B1FInspectionItem.batch_ragic_id == b.ragic_id
...
def get_batch_detail(
    ...
    return B1FInspectionBatchDetail(
...
def list_items(
    ...
    q = db.query(B1FInspectionItem)
    if batch_id:
        q = q.filter(B1FInspectionItem.batch_ragic_id == batch_id)
    items = q.order_by(B1FInspectionItem.batch_ragic_id, B1FInspectionItem.seq_no).all()
    result = []
    for it in items:
...
@router.get("/stats", summary="全站統計", response_model=B1FInspectionStats)
def get_stats(db: Session = Depends(get_db)):
    today     = date.today()

    latest_batch = db.query(B1FInspectionBatch).order_by(
        B1FInspectionBatch.inspection_date.desc()
    ).first()
    ...
    if latest_batch:
        items = db.query(B1FInspectionItem).filter(
            B1FInspectionItem.batch_ragic_id == latest_batch.ragic_id
        ).order_by(B1FInspectionItem.seq_no).all()
    ...
    for i in range(6, -1, -1):
        ...
        day_batches = db.query(B1FInspectionBatch).filter(
            B1FInspectionBatch.inspection_date == d_str
        ).all()
    ...
    total_batches_7d = db.query(B1FInspectionBatch).filter(
        B1FInspectionBatch.inspection_date >= week_ago
    ).count()

    return B1FInspectionStats(
...
def get_item_history(
    ...
    today = date.today()
    rows = (
        db.query(B1FInspectionItem, B1FInspectionBatch)
        .join(B1FInspectionBatch, B1FInspectionItem.batch_ragic_id == B1FInspectionBatch.ragic_id)
        .filter(B1FInspectionItem.item_name == item_name)
        .order_by(B1FInspectionBatch.inspection_date.desc())
        .all()
    )
    if not rows:
...
@router.get("/debug/ragic-raw", summary="[除錯] 顯示 Ragic Sheet 4 原始欄位", dependencies=[Depends(require_roles("system_admin", "module_manager"))])
async def debug_ragic_raw():
    from app.services.b1f_inspection_sync import (
        B1F_SERVER_URL, B1F_ACCOUNT, B1F_SHEET_PATH,
