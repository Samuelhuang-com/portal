"""
日曜核准請購單月報表 API Router

端點：
  GET  /api/v1/nichiyo-purchase-report/approved/orders           — 請購單清單（訂單級，分頁）
  GET  /api/v1/nichiyo-purchase-report/approved/orders/{id}      — 單筆詳情（含品項）
  GET  /api/v1/nichiyo-purchase-report/approved/monthly          — 月報明細（品項級，分頁）
  GET  /api/v1/nichiyo-purchase-report/approved/summary          — KPI 統計
  GET  /api/v1/nichiyo-purchase-report/approved/departments      — 部門彙總表
  GET  /api/v1/nichiyo-purchase-report/approved/export           — Excel 匯出（RFC 5987，B08 修正）
  GET  /api/v1/nichiyo-purchase-report/approved/available-months — 有資料的年月清單
  GET  /api/v1/nichiyo-purchase-report/config/departments        — 部門下拉
  GET  /api/v1/nichiyo-purchase-report/config/account-categories — 會科下拉
  POST /api/v1/nichiyo-purchase-report/sync                      — 手動觸發同步
  GET  /api/v1/nichiyo-purchase-report/sync/status               — 同步狀態
  POST /api/v1/nichiyo-purchase-report/admin/reparse-purchase-no — 就地修正 null 單號

權限：nichiyo_purchase.view / nichiyo_purchase.export / nichiyo_purchase.admin
"""
import asyncio
import io
...
_PERM        = "nichiyo_purchase.view"
_PERM_EXPORT = "nichiyo_purchase.export"
_PERM_ADMIN  = "nichiyo_purchase.admin"
_COMPANY     = "日曜"
...
def _apply_dept_filter(q, department: Optional[str]):
    if department:
        return q.filter(NichiyoPurchaseRequest.department_display == department)
    return q


def _apply_account_filter(q, account_category: Optional[str]):
    if account_category:
        return q.filter(NichiyoPurchaseRequest.account_category == account_category)
    return q


def _apply_search(q, keyword: Optional[str]):
    """B09：全文搜尋（說明/單號/申請人/廠商）"""
    if keyword:
        like = f"%{keyword}%"
        return q.filter(
            NichiyoPurchaseRequest.description.ilike(like)
            | NichiyoPurchaseRequest.purchase_no.ilike(like)
            | NichiyoPurchaseRequest.applicant.ilike(like)
            | NichiyoPurchaseRequest.vendor1.ilike(like)
        )
    return q


def _ragic_url(order: NichiyoPurchaseRequest) -> str:
    """B11：使用 list_path（記錄所在 Sheet），不是 detail_path（API 用）"""
...
def _order_to_dict(order: NichiyoPurchaseRequest) -> dict:
    return {
        "id":                 order.id,
...
def _format_item_row(order: NichiyoPurchaseRequest, item: NichiyoPurchaseRequestItem) -> dict:
    return {
        "order_id":           order.id,
...
def get_approved_orders(
    ...
    q:                Optional[str] = Query(None, description="關鍵字（說明/單號/申請人/廠商）"),
    ...
    """請購單清單（訂單級，分頁）"""
    base_q = db.query(NichiyoPurchaseRequest).filter(
        NichiyoPurchaseRequest.company == _COMPANY
    )
    ...
    base_q = base_q.order_by(NichiyoPurchaseRequest.approved_date.desc(),
                              NichiyoPurchaseRequest.id.desc())
    ...
    if order_ids:
        vrows = (
            ...
            .filter(
                ...
                NichiyoPurchaseRequestItem.selected_vendor.isnot(None),
                NichiyoPurchaseRequestItem.selected_vendor != "",
            )
    ...
    def _order_dict_with_vendors(o: NichiyoPurchaseRequest) -> dict:
        d = _order_to_dict(o)
    ...
    return {
        ...
        "page":     page,
        "per_page": per_page,
        "items":    [_order_dict_with_vendors(o) for o in rows],
    }
...
def get_order_detail(
    ...
    """單筆請購單詳情（含品項列表）"""
    order = db.query(NichiyoPurchaseRequest).filter(
        NichiyoPurchaseRequest.id == order_id,
        NichiyoPurchaseRequest.company == _COMPANY,
    ).first()
    if not order:
        raise HTTPException(status_code=404, detail="找不到此請購單")

    items = (
        db.query(NichiyoPurchaseRequestItem)
        .filter(NichiyoPurchaseRequestItem.order_id == order_id)
        .order_by(NichiyoPurchaseRequestItem.seq)
        .all()
    )
    def item_dict(i: NichiyoPurchaseRequestItem) -> dict:
        return {
            "id":                  i.id,
            "seq":                 i.seq,
...
def get_monthly_report(
    ...
    """月報明細（品項級，分頁）"""
    order_q = db.query(NichiyoPurchaseRequest).filter(
        NichiyoPurchaseRequest.company == _COMPANY
    )
    ...
    order_ids = [r.id for r in order_q.with_entities(NichiyoPurchaseRequest.id).all()]
    if not order_ids:
        return {"total": 0, "page": page, "per_page": per_page, "items": []}

    item_q = (
        db.query(NichiyoPurchaseRequestItem, NichiyoPurchaseRequest)
        .join(NichiyoPurchaseRequest,
              NichiyoPurchaseRequestItem.order_id == NichiyoPurchaseRequest.id)
        .filter(NichiyoPurchaseRequestItem.order_id.in_(order_ids))
        .order_by(NichiyoPurchaseRequest.approved_date.desc(),
                  NichiyoPurchaseRequest.purchase_no,
                  NichiyoPurchaseRequestItem.seq)
    )
...
def get_summary(
    ...
    item_count = (
        db.query(func.count(NichiyoPurchaseRequestItem.id))
        .filter(NichiyoPurchaseRequestItem.order_id.in_(order_ids))
        .scalar() or 0
    )
...
def get_departments(
    ...
    rows = (
        base_q.with_entities(
            ...
            func.count(NichiyoPurchaseRequest.id).label("order_count"),
            func.sum(NichiyoPurchaseRequest.amount).label("total_amount"),
            func.sum(NichiyoPurchaseRequest.amount_tax).label("total_tax"),
        )
        .group_by(NichiyoPurchaseRequest.department_display)
        .order_by(func.sum(NichiyoPurchaseRequest.amount).desc())
        .all()
    )
    return [
        {
...
def export_excel(
    ...
    """Excel 匯出（品項級別）B08：RFC 5987 filename* 編碼，避免 Latin-1 UnicodeEncodeError"""
    try:
        import openpyxl
    ...
    headers = [
        "公司", "部門", "請購單號", "申請日期", "核准日期",
        "申請人", "說明", "會科", "全案小計（未稅）", "營業稅", "備註",
        "項次", "品名", "數量", "單位", "擬定廠商", "擬定單價", "擬定金額",
        "Ragic連結",
    ]
    ...
    col_widths = [8, 10, 18, 12, 12, 10, 35, 18, 14, 10, 20, 6, 25, 8, 8, 18, 12, 12, 45]
...
def get_available_departments(
    ...
    rows = (
        db.query(NichiyoPurchaseRequest.department_display)
        .filter(NichiyoPurchaseRequest.company == _COMPANY)
        .distinct()
        .order_by(NichiyoPurchaseRequest.department_display)
        .all()
    )
    # B02：sorted 防 None
...
def get_account_categories(
    ...
    rows = (
        db.query(NichiyoPurchaseRequest.account_category)
        .filter(
            NichiyoPurchaseRequest.company == _COMPANY,
            NichiyoPurchaseRequest.account_category.isnot(None),
...
async def trigger_sync(
    ...
    from app.services.nichiyo_purchase_request_sync import sync_all, sync_list_only
    ...
    return {"status": "started", "message": f"日曜請購同步已啟動（full_resync={full_resync}）"}
...
def get_sync_status(
    ...
    recent_logs = (
        ...
        .filter(ModuleSyncLog.module_name == "日曜核准請購單")
    ...
    dept_rows = (
        db.query(
            NichiyoPurchaseRequest.department_display,
            func.count(NichiyoPurchaseRequest.id).label("total"),
        )
        .filter(NichiyoPurchaseRequest.company == _COMPANY)
        .group_by(NichiyoPurchaseRequest.department_display)
        .all()
    ...
    synced_rows = (
        db.query(
            NichiyoPurchaseRequest.department_display,
            func.count(NichiyoPurchaseRequest.id).label("synced"),
        )
        .filter(
            NichiyoPurchaseRequest.company == _COMPANY,
            NichiyoPurchaseRequest.detail_synced == True,
        )
        .group_by(NichiyoPurchaseRequest.department_display)
        .all()
    )
    ...
    pending_count = (
        db.query(func.count(NichiyoPurchaseRequest.id))
        .filter(
            NichiyoPurchaseRequest.company == _COMPANY,
...
def get_audit_anomalies(
    ...
    """日曜請購單資料異常稽核列表（分頁）。"""
    from app.services.audit_service import get_nichiyo_anomalies
    return get_nichiyo_anomalies(
        db,
...
def get_audit_summary(
    ...
    """日曜請購單資料異常稽核 KPI 摘要。"""
    from app.services.audit_service import get_nichiyo_audit_summary
    return get_nichiyo_audit_summary(
        db,
        year_month=year_month,
        year_month_from=year_month_from,
        year_month_to=year_month_to,
...
# ── POST /admin/reparse-purchase-no ──────────────────────────────────────────

@router.post("/admin/reparse-purchase-no")
def reparse_purchase_no(
    db: Session = Depends(get_db),
    _:  object  = Depends(require_permission(_PERM_ADMIN)),
):
    """
    從 raw_data_json 就地修復 purchase_no（含 regex fallback）。
    回傳 still_null：修正後仍為空的記錄，附上欄位清單供診斷。
    """
    import json as _json
    from app.services.nichiyo_purchase_request_sync import _pick_purchase_no, LIST_FIELD_CANDIDATES
    ...
    records = (
        ...
        .filter(NichiyoPurchaseRequest.raw_data_json.isnot(None))
        .all()
    )
    ...
    for rec in records:
        total += 1
        try:
            raw    = _json.loads(rec.raw_data_json)
            new_no = _pick_purchase_no(raw, LIST_FIELD_CANDIDATES["purchase_no"]) or None
            if new_no != rec.purchase_no:
                rec.purchase_no = new_no
                updated += 1
            if not new_no:
                still_null.append({
                    "id":              rec.id,
...
def get_combined_departments(
    ...
    """
    from app.models.nichiyo_claim_request import NichiyoClaimRequest
