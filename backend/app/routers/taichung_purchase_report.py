"""
台中核准請購單月報表 API Router（2026-09-16 完整複製自樂群 purchase_report.py）

與樂群差異：資料表 taichung_purchase_requests、company 預設「台中」、權限
taichung_purchase_report_view、稽核走 taichung_audit_service（R08 同部門內才算重複）、
額外欄位（主旨／31事由／擬定廠商編號名稱／擬定受款資訊／幣別／品項應稅金額）、
/debug/* 兩支診斷端點改為需要 taichung_purchase_report_manage（原始資料含銀行帳號）。

端點：
  GET  /api/v1/taichung-purchase-report/approved/monthly      — 月報表明細（品項級別，分頁）
  GET  /api/v1/taichung-purchase-report/approved/summary      — KPI 統計（9 項，主單級別）
  GET  /api/v1/taichung-purchase-report/approved/departments  — 部門彙總表
  GET  /api/v1/taichung-purchase-report/approved/export       — Excel 匯出（品項級別）
  POST /api/v1/taichung-purchase-report/sync                  — 手動觸發同步（Master + Detail）
  GET  /api/v1/taichung-purchase-report/sync/status           — 同步狀態查詢

權限：
  開發期間：system_admin_only（只有 system_admin 可存取）
  上線後：taichung_purchase_report_view（透過角色管理介面設定）
"""
import io
import asyncio
...
_PERM = "taichung_purchase_report_view"
_PERM_MANAGE = "taichung_purchase_report_manage"
...
):  # 台中：原始資料含銀行帳號，改為需要管理權限（使用者 2026-09-16 裁示）
...
def _format_item_row(order: ApprovedPurchaseRequest, item: ApprovedPurchaseRequestItem) -> dict:
    ...
    return {
        ...
        "reason_31":          order.reason_31,
        "final_vendor_no":    order.final_vendor_no,
        "final_vendor_name":  order.final_vendor_name,
        "final_payee":        order.final_payee,
        "final_bank_name":    order.final_bank_name,
        "final_bank_account": order.final_bank_account,
        "final_bank_code":    order.final_bank_code,
...
def export_excel(
    ...
    # 台中額外欄位（放在 Ragic連結 之前，既有欄位位置不動，數字格式欄 index 維持）
    EXTRA_COLS  = ["應稅金額", "主旨", "31事由", "擬定廠商編號", "擬定廠商名稱",
                   "擬定受款者", "擬定受款銀行", "擬定匯款帳號", "擬定銀行代號", "幣別"]
    headers     = MASTER_COLS + ITEM_COLS + EXTRA_COLS + ["Ragic連結"]
    ...
    for row_idx, (item, order) in enumerate(rows, 2):
        data = [
            ...
            item.taxable_amount if item.taxable_amount is not None else "",
            order.subject or "",
            order.reason_31 or "",
            order.final_vendor_no or "",
            order.final_vendor_name or "",
            order.final_payee or "",
            order.final_bank_name or "",
            order.final_bank_account or "",
            order.final_bank_code or "",
            order.currency or "",
            _ragic_url(order),
        ]
        for col_idx, val in enumerate(data, 1):
            ...
            if isinstance(val, int) and col_idx in (9, 10, 17, 18, 19):
                cell.number_format = '#,##0'

    col_widths = [8, 10, 18, 12, 18, 10, 35, 18, 14, 10, 20, 6, 25, 8, 8, 18, 12, 12, 12, 30, 30, 14, 20, 20, 20, 16, 12, 8, 45]
    ...
    filename_cn = f"台中核准請購單月報表_{label}.xlsx"
    # filename= 只能是 Latin-1，中文（如「2026年度」）必須轉成 ASCII 後備值，
    # 否則 Starlette 組 header 時會丟 UnicodeEncodeError（整支匯出 500）。
    filename_safe = f"taichung_purchase_report_{ascii_filename_label(label)}.xlsx"
...
async def trigger_sync(
    ...
    """
    手動觸發台中核准請購單雙層同步（背景執行，立即回傳 202）。
    full_resync=True 時強制重新同步所有 Detail 資料。
    """
    from app.services.taichung_purchase_request_sync import sync_from_ragic
...
def get_sync_status(
    ...
    recent_logs = (
        ...
        .filter(ModuleSyncLog.module_name == "台中核准請購單")
...
def _order_to_dict(o: ApprovedPurchaseRequest) -> dict:
    return {
        ...
        # 台中額外欄位
        "subject":            o.subject,
        "reason_31":          o.reason_31,
        "final_vendor_no":    o.final_vendor_no,
        "final_vendor_name":  o.final_vendor_name,
        "final_payee":        o.final_payee,
        "final_bank_name":    o.final_bank_name,
        "final_bank_account": o.final_bank_account,
        "final_bank_code":    o.final_bank_code,
        "currency":           o.currency,
...
# ─────────────────────────────────────────────────────────────────────────────
# GET /debug/raw-keys — 列出第一筆的 raw_data_json 所有欄位 key（台中：需 manage 權限）
# ─────────────────────────────────────────────────────────────────────────────

@router.get("/debug/raw-keys")
def debug_raw_keys(
    db:         Session = Depends(get_db),
    department: str = Query("客務部"),
    limit:      int = Query(3),
    _:          object = Depends(require_permission(_PERM_MANAGE)),
):
...
def get_purchase_audit_anomalies(
    ...
    from app.services.taichung_audit_service import get_anomalies
...
def get_purchase_audit_summary(
    ...
    """請購資料異常各規則計數 KPI。"""
    from app.services.taichung_audit_service import get_audit_summary
