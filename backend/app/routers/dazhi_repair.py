...
async def verify_count(db: Session = Depends(get_db)):
    ...
    # ── Ragic 即時筆數 ────────────────────────────────────────────────────────
    from app.services.dazhi_repair_service import _get_adapter
    from app.core.config import settings as _s
    adapter = _get_adapter()
    try:
        ragic_count = await adapter.fetch_count(
            extra_params={"PAGEID": _s.RAGIC_DAZHI_REPAIR_PAGEID}
        )
    except Exception as exc:
...
async def verify_diff(db: Session = Depends(get_db)):
    ...
    base_url = f"https://ap12.ragic.com/soutlet001/lequn-public-works/8"
