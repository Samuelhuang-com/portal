"""
台中 請購單 + 請款單 整合總表 API Router（2026-09-16 完整複製自樂群 combined_report.py，
資料表改 taichung_*，company 預設「台中」，權限同樂群 system_admin_only）

請購單 + 請款單 整合總表 API Router（方案 B：後端合併端點）

端點：
  GET /api/v1/taichung-combined-report/orders   — 請購 + 請款 合併清單（分頁）
  GET /api/v1/taichung-combined-report/summary  — 請購 + 請款 合計 KPI
  GET /api/v1/taichung-combined-report/departments — 部門雙色統計（請購/請款並排）
...
from app.models.taichung_purchase_request import TaichungPurchaseRequest as ApprovedPurchaseRequest
from app.models.taichung_claim_request import TaichungClaimRequest as ApprovedClaimRequest
