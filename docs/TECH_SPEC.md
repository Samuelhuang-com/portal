...
| v1.39.0 | 2026-04-24 | **IHG 客房保養模組**：`ihg_rm_master` + `ihg_rm_detail` + `ihg_rm_section` 三表；section 於 Ragic 同步時自動拆出 V/X 值域，供月報矩陣 `/section-matrix`；年度保養矩陣 `/matrix`（房號×月份）；KPI `/stats`；Menu 位置：飯店管理→2.IHG客房保養，含「年度矩陣」+「客房保養表明細」兩 TAB |
...
| v1.44.0 | 2026-04-30 | **飯店管理 Dashboard**：前端純前端 Normalize Adapter 層（6 個 `adapt*()` 函式，統一 `NormalizedSource` 介面）；不新增 API，沿用 `/room-maintenance-detail/maintenance-stats`、`/periodic-maintenance/stats`、`/ihg-room-maintenance/stats`、`/hotel-daily-inspection/dashboard/summary`、`/security/dashboard/summary`、`/dazhi-repair/dashboard` 6 個現有端點；`pages/HotelMgmtDashboard/index.tsx`；route `/hotel/overview`；Menu hotel 群組置頂 |
| v1.52.3 | 2026-05-01 | **三大監控模組 Dashboard 查詢月份**：新增 `app/core/date_utils.py`（`get_month_range` / `to_ragic_year_month` / `current_month_str`）；`hotel-meter-readings /dashboard/summary` 新增 `month` param（YYYY-MM），修正月份過濾邏輯；`mall-facility-inspection` 新端點 `/dashboard/monthly-summary`；`full-building-inspection` 新端點 `/dashboard/monthly-summary`（空結構，待 sync 實作）；前端三頁 Dashboard Tab 改用月份選擇器、URL `?month=YYYY-MM`、KPI 依月份重算、新增 `api/fullBuildingInspection.ts` |
| v1.54.0 | 2026-05-03 | **決策駕駛艙（Decision Cockpit）Phase 1**：前端新增 `pages/DecisionCockpit/index.tsx`（9 TAB 懶載入框架；月份選擇器；tabProps 統一介面）；`tabs/Tab{Overview,Hotel,Mall,Repair,Personnel,RiskRadar,Trend,Briefing,DataQuality}.tsx` 9 個 TAB stub；`navLabels.ts` + `MainLayout.tsx` + `router/index.tsx` 掛載；路由 `/decision-cockpit`；permissionKey `decision_cockpit_view`；不新增後端 API，Phase 2+ 全部重用既有 22 個端點；規劃文件存於 `docs/decision-cockpit/` |
| v1.45.0 | 2026-04-30 | **飯店管理 Dashboard Tab B/C/D/人員排名**：後端新增 `routers/hotel_overview.py`（`GET /api/v1/hotel/daily-hours`、`/hotel/monthly-hours`、`/hotel/person-hours`）；六來源聚合（客房保養管理/飯店週期保養/IHG客房保養/飯店每日巡檢/保全巡檢/大直工務部）；IHG 無工時欄位以每筆 0.5hr 固定估算；前端新增 `api/hotelOverview.ts`；Dashboard 新增 4 個懶載入 Tab（B.每日累計/C.每月累計/D.人員工時%/人員排名）；useRef loadedTabs 避免重複請求；格式與 mall_overview 完全一致 |
| v1.59.16 | 2026-05-13 | **核准請購單月報表 Phase 3（前端）**：`purchaseReport.ts`（7 API函數+TypeScript型別）；`PurchaseReport/index.tsx`（KPI×6卡、月報明細Tab、部門統計Tab、同步狀態Tab(admin)、匯出Excel、年月/部門篩選器）；sidebar「請購單報表」群組（budget前）；路由`/purchase-report/monthly`；permissionKey `system_admin_only` |
| v1.59.15 | 2026-05-13 | **核准請購單月報表 Phase 1**：`approved_purchase_requests`（主單）＋`approved_purchase_request_items`（品項子表）雙表架構；Master+Detail 雙層同步（N+1+100ms延遲+批次50+detail_synced增量旗標）；`DEPT_DISPLAY_MAP`（停管部映射）；`/api/v1/purchase-report/` 路由群（品項級別月報+KPI+Excel匯出）；開發期 permissionKey=`system_admin_only` |
| v1.58.0 | 2026-05-10 | **專案知識圖譜（graphify 整合）**：後端 `routers/knowledge_graph.py`（GET /status / POST /generate / GET /result）+ `services/graphify_runner.py`（subprocess 封裝、狀態 JSON、候選 HTML 偵測）；FastAPI StaticFiles 掛載 `/kg-files/`；前端 `pages/Settings/KnowledgeGraph/index.tsx`（iframe 嵌入 + 3s 輪詢 + 進度條）+ `api/knowledgeGraph.ts`；route `/settings/knowledge-graph`；permissionKey `system_admin_only` |
...
| 工具 | 安裝指令 | 用途 |
|------|---------|------|
| graphify | `pip install graphifyy` + `graphify install` | 專案知識圖譜：AST + LLM 分析整個 portal，輸出互動式 HTML |
...
### 服務層共用工具（app/services/）

| 檔案 | 功能 |
|------|------|
| `time_utils.py` | `parse_minutes(start, end) -> int`：解析 HH:MM 時間差值，支援跨日修正；供 `hotel_overview.py` 與 `mall_overview.py` 共用，取代各自重複的 `_parse_minutes` 函式 |
...
| graphify 圖譜 HTML 以 StaticFiles 掛載供 iframe 存取 | iframe 無法傳送 Bearer token auth header，若透過 `/api/v1/knowledge-graph/result` 回傳則前端無法直接嵌入；改用 `/kg-files/` StaticFiles 掛載（無驗證）；可接受：輸出為程式碼視覺化（非敏感業務資料），且本地部署環境網路已隔離 |
| graphify 採 BackgroundTask + status JSON 輪詢 | graphify 分析大型專案需 2–5 分鐘，不適合同步 HTTP 請求；以 FastAPI BackgroundTasks 非同步執行，狀態寫入 `_status.json`；前端每 3 秒輪詢 `/status`；避免引入 Celery/Redis 等額外依賴 |
...
| Menu 權限採「角色→permission_key→三層控制」RBAC | 既有系統只有 system_admin 特例，擴展為 role_permissions 多對多表；system_admin 回傳 ["*"] 萬用符，其他角色從 DB 取；前端 filterMenuByPermissions + PermissionGuard + 後端 require_permission 三層保護；新模組開發期間 permissionKey='system_admin_only'，測試後手工授予 |
