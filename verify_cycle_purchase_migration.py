...
EXPECTED = [
    ...
    (
        ...
        "第六次調整：供應商主檔改為鏡像合約模組 vendors（CHANGELOG [1.90.30]）"
        "　⚠️ 這兩個欄位缺了會讓所有查供應商的端點 500 —— 供應商主檔、採購單、"
        "驗收、付款、彙整單整片變空白，畫面上看起來像「資料庫連錯檔案、資料不見了」，"
        "極容易誤判。2026-08-10 起後端啟動時會自動補這兩欄，所以若這裡顯示缺少，"
        "通常代表**後端還沒用新版程式碼重啟過**",
    ),
]
